...
const MODELS: [&[u8]; 10] = [
    b"none", b"no_vocab", b"llama", b"gemma4", b"gpt2", b"bert", b"t5", b"rwkv",
    b"plamo2", b"future",
];

const PRE_ALIASES: [&[u8]; 61] = [
    b"", b"default", b"llama3", b"llama-v3", b"llama-bpe", b"falcon3", b"falcon-h1",
    b"pixtral", b"midm-2.0", b"lfm2", b"jina-v5-nano", b"jais2", b"dbrx", b"smaug",
    b"deepseek-llm", b"deepseek-coder", b"deepseek-v3", b"youtu", b"falcon", b"mpt",
    b"starcoder", b"gpt2", b"gpt-2", b"jais", b"refact", b"command-r", b"qwen2",
    b"qwen2.5", b"qwen35", b"stablelm2", b"olmo", b"poro", b"chatglm4", b"viking",
    b"tekken", b"smollm", b"codeshell", b"bloom", b"gpt3-finnish", b"exaone",
    b"exaone4", b"exaone-moe", b"chameleon", b"minerva", b"megrez", b"gpt4o", b"gpt-4o",
    b"tiny-aya", b"superbpe", b"trillion", b"granite-docling", b"bailingmoe", b"seed-coder",
    b"hunyuan", b"hunyuan-dense", b"joyai-llm", b"kimi-k2", b"grok-2", b"afmoe",
    b"minimax-m2", b"solar-open",
];
...
fn scalar_payload(kind: u32, source: &[u8]) -> Vec<u8> {
    ...
    match kind {
        ...
        8 => {
            ...
            push_string(&mut bytes, source.get(..source.len().min(8)).unwrap_or(source));
            bytes
        }
        9 => {
            let mut bytes = Vec::new();
...
fn constructed_input(data: &[u8]) -> Vec<u8> {
    ...
    let mut fixture = Fixture::new()
        ...
        .array(b"tokenizer.precompiled_charsmap", U8, tail, u64::try_from(tail.len()).unwrap());
    fixture = match scenario {
        ...
        _ => Fixture::new()
            .string(b"tokenizer.model", model)
            .array(b"tokenizer.tokens", STRING, &[], u64::MAX),
    };
    fixture
        .scalar(b"tokenizer.add_bos_token", BOOL, [data.first().copied().unwrap_or(0) & 1])
