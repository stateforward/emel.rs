...
fuzz_target!(|data: &[u8]| {
    ...
    for (index, action) in data.iter().copied().take(MAX_ACTIONS).enumerate() {
        ...
        match action % 7 {
            ...
            2 => bind_exact_if_bounded(&mut loader, requirements),
            3 => bind_small_capacity(&mut loader, action),
            4 => parse_and_validate(&mut loader, data),
            5 => parse_and_validate(&mut loader, shortened),
...
fn bounded_probe(loader: &mut Loader, data: &[u8]) -> Option<ProbeDone> {
    loader
        .probe(data)
        .ok()
        .filter(|requirements| requirements_are_bounded(*requirements))
}
...
fn bind_exact_if_bounded(loader: &mut Loader, requirements: Option<ProbeDone>) {
    if requirements.is_some() {
        ...
        let _ = loader.bind_with_capacity(0, 0, 0);
    }
}

fn bind_small_capacity(loader: &mut Loader, action: u8) {
    let units = usize::from(action >> 3);
    let _ = loader.bind_with_capacity(units * 128, units, units);
}

fn parse_and_validate(loader: &mut Loader, data: &[u8]) {
