...
fn exercise_queries(loader: &mut Loader, key: &[u8], index: u64) -> u64 {
    ...
    hash_result(
        &mut hash,
        loader.process_event(ReadUnsigned::new(key)),
        |hash, value| {
    ...
    hash_result(
        &mut hash,
        loader.process_event(ReadSigned::new(key)),
        |hash, value| {
            hash_bytes(hash, &value.to_le_bytes());
        },
    );
    hash_result(
        &mut hash,
        loader.process_event(ReadF32::new(key)),
        |hash, value| {
    ...
    hash_result(
        &mut hash,
        loader.process_event(ReadF64::new(key)),
        |hash, value| {
            hash_bytes(hash, &value.to_bits().to_le_bytes());
        },
    );
    hash_result(
        &mut hash,
        loader.process_event(ReadBool::new(key)),
        |hash, value| {
    ...
    let visit = loader.process_event(VisitUnsignedArray::new(
        key,
        |visited_index: u32, value: u64| {
