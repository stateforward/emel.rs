...
for expected in \
  ...
  '# benchmark_validation: typed loader done checked each iteration, target equals source after measurement'; do
  ...
  ' "$artifact"; then
    echo "error: $label must contain exactly one loader benchmark field: $expected" >&2
    exit 1
  fi
done

max_cross_lane_ratio="${EMEL_LOADER_BENCH_MAX_RUST_VS_CPP_RATIO:-2.0}"
if ! awk -v expected_iterations="$expected_iterations" -v expected_runs="$expected_runs" \
  ...
  {
    rows += 1
    if ($1 != "io/loader/read_copy_1mib" || seen_case[$1]++) { invalid = 1 }
    delete fields
    delete values
  ...
  END { exit !(rows == 1 && !invalid && seen_case["io/loader/read_copy_1mib"] == 1) }
' "$artifact"; then
  echo "error: $label has an invalid loader benchmark result schema" >&2
  exit 1
fi
