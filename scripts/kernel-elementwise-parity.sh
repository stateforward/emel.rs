...
BUILD_DIR="${EMEL_KERNEL_ELEMENTWISE_PARITY_BUILD_DIR:-$ROOT_DIR/.artifacts/kernel-elementwise-parity}"
SNAPSHOT="${EMEL_KERNEL_ELEMENTWISE_PARITY_SNAPSHOT:-$ROOT_DIR/snapshots/parity/kernel-elementwise/manifest.txt}"
...
usage() {
  cat <<'USAGE'
usage: scripts/kernel-elementwise-parity.sh [OPTIONS]

Default mode runs the live C++/Rust comparison, updates the checked-in
snapshot, and verifies the resulting snapshot. Use --snapshot-only to omit
the live build and compare the checked-in snapshot against a fresh observer
manifest, --live-only to skip snapshot work, or --no-update to preserve the
existing snapshot.
...
if $LIVE; then
  ...
  RUST_OBSERVER="$CARGO_TARGET_DIR/debug/emel-kernel-elementwise-parity"
...

{
  ...
  printf 'config=Release,cxx20,dense-contiguous-f32,public-rust-actor,public-cpp-kernel\n'
  printf 'result=match\n'
...
echo "Kernel dense F32 elementwise parity passed (emel.cpp $SOURCE_COMMIT)"
