...
BUILD_DIR="${EMEL_TARGET_AARCH64_Q6_PACKED_LIVE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/kernel-goal/q6-packed}"
SNAPSHOT="${EMEL_TARGET_AARCH64_Q6_PACKED_SNAPSHOT:-$ROOT_DIR/snapshots/parity/kernel-target-aarch64-q6-packed/manifest.txt}"
...
for argument in "$@"; do
  case "$argument" in
    ...
    --help|-h) printf '%s\n' 'usage: scripts/kernel-target-aarch64-q6-packed-parity.sh [--snapshot|--no-snapshot] [--update|--no-update] [--snapshot-only|--live-only|--update-only]'; exit 0 ;;
...
if $LIVE; then
  cmake -S "$ROOT_DIR/tools/emel-kernel-target-aarch64-q6-packed-reference" -B "$BUILD_DIR/cpp-build" -G Ninja \
  ...
  cmake --build "$BUILD_DIR/cpp-build" --target emel-kernel-target-aarch64-q6-packed-reference >"$BUILD_DIR/logs/cmake-build.log" 2>&1
  cargo run --locked --offline --manifest-path "$ROOT_DIR/tools/emel-kernel-target-aarch64-q6-packed-parity/Cargo.toml" --quiet \
    >"$BUILD_DIR/rust.out" 2>"$BUILD_DIR/logs/rust.stderr"
  "$BUILD_DIR/cpp-build/emel-kernel-target-aarch64-q6-packed-reference" >"$BUILD_DIR/cpp.out" 2>"$BUILD_DIR/logs/cpp.stderr"
else
...
printf 'AArch64 target q6 packed live parity passed (emel.cpp %s)\n' "$SOURCE_COMMIT"
