...
INVENTORY_SHA256=d90b859b4c3e21da37444fee41020926b5085bad4a47ce4ff8876c9ca36333bc
OWNER_MAP_SHA256=560059a1be0b84dd0c462d180e7bd4550bb0e27f8597414d64073ba106d58c87
...
if [[ ! -f "$EMEL_CPP_SOURCE/$QWEN3_RELATIVE" ]]; then
  echo "error: pinned Qwen3 fixture unavailable: $EMEL_CPP_SOURCE/$QWEN3_RELATIVE" >&2
  echo "fixture_qwen3_status=unavailable" >&2
  exit 1
fi
...
proof_token="$(printf '%s\n' "$SOURCE_COMMIT" "$DETAIL_HEADER_BLOB" \
  "$DETAIL_IMPLEMENTATION_BLOB" "$LIFECYCLE_TEST_BLOB" "$QWEN3_SHA256" | sha256_text)"
...
RUST_OBSERVER="${CARGO_TARGET_DIR:-$ROOT_DIR/target}/debug/examples/qwen3_observer"
...
"$RUST_OBSERVER" --fixture "$EMEL_CPP_SOURCE/$QWEN3_RELATIVE" >"$BUILD_DIR/rust.fixture"
