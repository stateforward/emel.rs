...
EMEL_KERNEL_CAPABILITY_SML_SOURCE="${EMEL_STATEFORWARD_SML_SOURCE:-$ROOT_DIR/../../sml/sml.cpp-worktrees/pinned-49207123cd3f39767764bae774932cb48623f92f}"
...
then checked-in snapshot verification.
  ...
  --suite=NAME     run all, gguf, io-read, io-mmap, io-staged-read, io-loader, model-tensor, model-data, model-catalog, token-profile, model-vocab, kernel, or kernel-capability (default: all)
...
if $RUN_GGUF || $RUN_IO_READ || $RUN_IO_MMAP || $RUN_IO_STAGED_READ || \
  ...
  $RUN_MODEL_VOCAB || $RUN_KERNEL || $RUN_KERNEL_CAPABILITY; then
...
if $RUN_GGUF; then
  ...
  RUST_RUNNER="$RUST_TARGET_DIR/debug/emel-gguf-parity"
...
run_io_mmap_parity() {
  ...
  cargo build --manifest-path "$ROOT_DIR/Cargo.toml" -p emel-io \
    --example mmap_parity
  local rust_target_dir="${CARGO_TARGET_DIR:-$ROOT_DIR/target}"
  local rust_runner="$rust_target_dir/debug/examples/mmap_parity"
...
run_model_catalog_parity() {
  ...
  if [[ ${#mode[@]} -eq 0 ]]; then
    ...
    EMEL_CPP_SOURCE_DIR="$EMEL_CPP_SOURCE" \
      "$ROOT_DIR/scripts/model-gemma4-parity.sh"
  else
    EMEL_CPP_SOURCE_DIR="$EMEL_CPP_SOURCE" \
    ...
    EMEL_CPP_SOURCE_DIR="$EMEL_CPP_SOURCE" \
      "$ROOT_DIR/scripts/model-gemma4-parity.sh" "${mode[0]}"
  fi
}
...
if $RUN_KERNEL; then
  kernel_args=()
  if $RUN_LIVE && $RUN_UPDATE; then
    kernel_args+=(--update)
  elif $RUN_LIVE; then
    kernel_args+=(--live-only)
  elif $RUN_UPDATE; then
    kernel_args+=(--update-only)
  else
    kernel_args+=(--snapshot-only)
  fi
  "$ROOT_DIR/scripts/kernel-parity-suite.sh" "${kernel_args[@]}"
fi
