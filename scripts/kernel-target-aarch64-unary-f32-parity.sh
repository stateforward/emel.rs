...
BUILD_DIR="${EMEL_TARGET_AARCH64_UNARY_F32_LIVE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/kernel-goal/target-aarch64-unary-f32-live}"
SNAPSHOT="${EMEL_TARGET_AARCH64_UNARY_F32_SNAPSHOT:-$ROOT_DIR/snapshots/parity/kernel-target-aarch64-unary-f32/manifest.txt}"
...
usage() {
  cat <<'USAGE'
usage: scripts/kernel-target-aarch64-unary-f32-parity.sh [OPTIONS]

Runs pinned C++ and public Rust AArch64 abs/neg/relu observers. Snapshot,
...
if $LIVE; then
  cmake -S "$ROOT_DIR/tools/emel-kernel-target-aarch64-unary-f32-reference" \
  ...
  cmake --build "$BUILD_DIR/cpp-build" \
    --target emel-kernel-target-aarch64-unary-f32-reference \
    >"$BUILD_DIR/logs/cmake-build.log" 2>&1
  cargo build --locked --offline -p emel-kernel-target-aarch64-unary-f32-parity \
    >"$BUILD_DIR/logs/cargo-build.log" 2>&1
  "$BUILD_DIR/cpp-build/emel-kernel-target-aarch64-unary-f32-reference" \
    >"$BUILD_DIR/cpp.out" 2>"$BUILD_DIR/cpp.stderr"
  "$CARGO_TARGET_DIR/debug/emel-kernel-target-aarch64-unary-f32-parity" \
...
sha256_file() { shasum -a 256 "$1" | awk '{print $1}'; }
{
  ...
  printf 'observer=split_pinned_cpp_aarch64_sm_and_public_rust_target_router\n'
  printf 'reference=../emel.cpp\n'
  cat "$BUILD_DIR/rust.out"
  printf 'source_detail_validate_span=src/emel/kernel/detail.hpp:1920-1926\n'
  printf 'source_aarch64_guard_span=src/emel/kernel/aarch64/guards.hpp:826-870\n'
  printf 'source_aarch64_action_span=src/emel/kernel/aarch64/actions.hpp:125-178,8379-8412,9062-9068\n'
  printf 'source_aarch64_transition_span=src/emel/kernel/aarch64/sm.hpp:1161-1178\n'
  ...
  printf 'reference_observer_sha256=%s\n' "$(sha256_file "$ROOT_DIR/tools/emel-kernel-target-aarch64-unary-f32-reference/main.cpp")"
  printf 'rust_observer_sha256=%s\n' "$(sha256_file "$ROOT_DIR/tools/emel-kernel-target-aarch64-unary-f32-parity/src/main.rs")"
  printf 'rust_module=crates/emel-kernels/src/aarch64/unary.rs\n'
  printf 'rust_router=crates/emel-kernels/src/aarch64/sm.rs\n'
  printf 'rust_tests=crates/emel-kernels/src/aarch64/unary.rs::tests\n'
  printf 'scope=NEON F32 abs/neg/relu over equal non-empty dense slices\n'
  printf 'body_tail_case=count=9 (two NEON vectors plus one scalar tail)\n'
  printf 'positive_cases=abs/neg/relu exact output bits\n'
  printf 'rejection_cases=zero_abs/zero_neg/zero_relu and unequal invalid_shape typed rejection\n'
  ...
  printf 'live_artifacts=.artifacts/kernel-goal/target-aarch64-unary-f32-live\n'
  printf 'residuals=SiLU and generic scalar unary operations are outside this claim; no portable/strided/quantized/non-F32 variants; no x86 live target claim\n'
...
printf 'AArch64 target unary F32 live parity passed (emel.cpp %s)\n' "$SOURCE_COMMIT"
