...
ARTIFACT_DIR="${EMEL_KERNEL_ROPE_BENCH_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/kernel-goal/rope-bench}"
SNAPSHOT="${EMEL_KERNEL_ROPE_BENCH_SNAPSHOT:-$ROOT_DIR/snapshots/bench/kernel-rope-aarch64.txt}"
...
for argument in "$@"; do
  case "$argument" in
    ...
    --help|-h) printf '%s\n' 'usage: scripts/kernel-rope-bench.sh [--snapshot|--no-snapshot] [--update|--no-update|--update-only]'; exit 0 ;;
...
cmake -S "$ROOT_DIR/tools/emel-kernel-rope-reference" -B "$ARTIFACT_DIR/target/cpp-bench" -G Ninja \
...
cmake --build "$ARTIFACT_DIR/target/cpp-bench" --target emel-kernel-rope-reference \
  >"$ARTIFACT_DIR/logs/cmake-build.log" 2>&1
cargo build --quiet --offline --locked --release --manifest-path "$ROOT_DIR/Cargo.toml" \
  -p emel-kernel-rope-parity >"$ARTIFACT_DIR/logs/cargo-build.log" 2>&1

EMEL_KERNEL_ROPE_BENCHMARK=1 "$ARTIFACT_DIR/target/cpp-bench/emel-kernel-rope-reference" \
  >"$ARTIFACT_DIR/bench-cpp.out"
"$CARGO_TARGET_DIR/release/emel-kernel-rope-parity" --benchmark >"$ARTIFACT_DIR/bench-rust.out"
cpp_ns="$(awk -F'[ =]' '/case=op_rope_norm/{print $4}' "$ARTIFACT_DIR/bench-cpp.out")"
rust_ns="$(awk -F'[ =]' '/case=op_rope_norm/{print $4}' "$ARTIFACT_DIR/bench-rust.out")"
[[ -n "$cpp_ns" && -n "$rust_ns" ]] || { printf 'error: malformed rope benchmark output\n' >&2; exit 1; }
...
' || {
  printf 'error: rope Rust/C++ ratio exceeds %sx\n' "$max_ratio" >&2; exit 1;
}
...
current="$ARTIFACT_DIR/bench-current.txt"
{
  ...
  printf '# benchmark_config: rope_norm_4d iterations=10000 warmup=1000\n'
  printf '# benchmark_operand: positive-contiguous-f32 public root route\n'
  printf '# benchmark_validation: both public observer lanes report output=4\n'
  printf 'kernel/rope/op_rope_norm rust_ns_per_dispatch=%s cpp_ns_per_dispatch=%s rust_vs_cpp_ratio=%.6f output=4\n' \
