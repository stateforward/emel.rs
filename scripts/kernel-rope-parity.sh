...
BUILD_DIR="${EMEL_KERNEL_ROPE_PARITY_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/kernel-goal/rope-parity}"
SNAPSHOT="${EMEL_KERNEL_ROPE_PARITY_SNAPSHOT:-$ROOT_DIR/snapshots/parity/kernel-rope/manifest.txt}"
...
usage() {
  cat <<'USAGE'
usage: scripts/kernel-rope-parity.sh [OPTIONS]

Runs the pinned C++ and public Rust RoPE observers, compares their output,
updates the snapshot, and verifies it by default. --snapshot-only reruns both
observers without updating the snapshot. Any --no-live mode fails closed.
...
cmake -S "$ROOT_DIR/tools/emel-kernel-rope-reference" \
...
cmake --build "$BUILD_DIR/target/cpp-build" \
  --target emel-kernel-rope-reference >"$BUILD_DIR/logs/cmake-build.log" 2>&1
cargo build --quiet --locked --manifest-path "$ROOT_DIR/Cargo.toml" \
  -p emel-kernel-rope-parity >"$BUILD_DIR/logs/cargo-build.log" 2>&1

CPP_OBSERVER="$BUILD_DIR/target/cpp-build/emel-kernel-rope-reference"
RUST_OBSERVER="$CARGO_TARGET_DIR/debug/emel-kernel-rope-parity"
...

{
  ...
  printf 'reference_tool_sha256=%s\n' \
    "$(sha256_file "$ROOT_DIR/tools/emel-kernel-rope-reference/main.cpp")"
  printf 'rust_observer_sha256=%s\n' \
    "$(sha256_file "$ROOT_DIR/tools/emel-kernel-rope-parity/src/main.rs")"
  printf 'config=Release,cxx20,positive-contiguous-f32,public-rust-rope-actor,reference-kernel\n'
  printf 'rust_execution=public_Kernel_process_event_to_owned_RopeKernel\n'
...
echo "Kernel RoPE parity passed (emel.cpp $SOURCE_COMMIT)"
