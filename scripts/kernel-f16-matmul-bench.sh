...
BUILD_DIR="${EMEL_KERNEL_F16_MATMUL_BENCH_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/kernel-goal/f16-matmul}"
SNAPSHOT_OVERRIDE="${EMEL_KERNEL_F16_MATMUL_BENCH_SNAPSHOT:-}"
...
for argument in "$@"; do
  case "$argument" in
    ...
    --help|-h)
      printf '%s\n' 'usage: scripts/kernel-f16-matmul-bench.sh [--snapshot|--no-snapshot] [--update|--no-update|--update-only]'
      exit 0
      ;;
...
for command in cargo cmake git rustc diff shasum; do
...
for entry in \
  ...
  [[ "$actual" == "$expected" ]] || {
    echo "error: pinned blob mismatch for $path: $actual" >&2
    exit 1
  }
done
...
cmake -S "$ROOT_DIR/tools/emel-kernel-f16-matmul-reference" \
  -B "$BUILD_DIR/target/f16-bench-cpp" -G Ninja \
  ...
  >"$BUILD_DIR/logs/bench-cmake-configure.log" 2>&1
cmake --build "$BUILD_DIR/target/f16-bench-cpp" \
  --target emel-kernel-f16-matmul-reference >"$BUILD_DIR/logs/bench-cmake-build.log" 2>&1
cargo build --quiet --offline --locked --release --manifest-path "$ROOT_DIR/Cargo.toml" \
  -p emel-kernel-f16-matmul-parity >"$BUILD_DIR/logs/bench-cargo-build.log" 2>&1
"$BUILD_DIR/target/f16-bench-cpp/emel-kernel-f16-matmul-reference" --benchmark >"$BUILD_DIR/bench-cpp.out"
"$CARGO_TARGET_DIR/release/emel-kernel-f16-matmul-parity" --benchmark >"$BUILD_DIR/bench-rust.out"
cpp_ns="$(awk -F'[ =]' '/case=op_mul_mat_f16/{print $4}' "$BUILD_DIR/bench-cpp.out")"
rust_ns="$(awk -F'[ =]' '/case=op_mul_mat_f16/{print $4}' "$BUILD_DIR/bench-rust.out")"
cpp_output="$(awk -F'[ =]' '/case=op_mul_mat_f16/{print $6}' "$BUILD_DIR/bench-cpp.out")"
rust_output="$(awk -F'[ =]' '/case=op_mul_mat_f16/{print $6}' "$BUILD_DIR/bench-rust.out")"
[[ -n "$cpp_ns" && -n "$rust_ns" && "$cpp_output" == 16 && "$rust_output" == 16 ]] || {
  echo 'error: malformed F16 matmul benchmark output' >&2; exit 1;
}
...
' || {
  echo "error: F16 matmul Rust/C++ ratio exceeds ${max_ratio}x" >&2; exit 1;
}
TARGET_ARCH="$(rustc --print cfg | awk -F'"' '/^target_arch=/{print $2; exit}')"
if [[ -n "$SNAPSHOT_OVERRIDE" ]]; then
  SNAPSHOT="$SNAPSHOT_OVERRIDE"
else
  SNAPSHOT="$ROOT_DIR/snapshots/bench/kernel-f16-matmul-${TARGET_ARCH}.txt"
fi
case "$TARGET_ARCH" in
  aarch64) ACTIVE_SM=865a9cc6ba6115382ed043c464f3d62bcd851357; ACTIVE_GUARDS=c25714566ec9a02679daef85089544575123408e; ACTIVE_ACTIONS=267d4f74e6e7498155c8535920322ffef2c02fb6 ;;
  x86_64) ACTIVE_SM=0b4d635ebbd0fbd52dbca8a2345547fb571205c8; ACTIVE_GUARDS=cb3dac8253f8417c9b44acff1de414f6d0a3a3cf; ACTIVE_ACTIONS=d45558f5eb96950f43c16a09d768cb4f382d6d61 ;;
...
current="$BUILD_DIR/bench-current.txt"
{
  ...
  printf '# benchmark_operand: dense explicit nonzero-stride F16-by-F16-to-F32 scalar double accumulation\n'
  printf '# benchmark_validation: both public observer lanes report output=16\n'
  printf 'kernel/f16_matmul/op_mul_mat_f16 rust_ns_per_dispatch=%s cpp_ns_per_dispatch=%s rust_vs_cpp_ratio=%.6f output=16\n' \
