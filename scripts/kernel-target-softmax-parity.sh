...
BUILD_DIR="${EMEL_TARGET_SOFTMAX_LIVE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/kernel-goal/target-softmax-live}"
SNAPSHOT="${EMEL_TARGET_SOFTMAX_SNAPSHOT:-$ROOT_DIR/snapshots/parity/kernel-target-softmax/manifest.txt}"
...
for argument in "$@"; do
  case "$argument" in
    ...
    --help|-h) printf '%s\n' 'usage: scripts/kernel-target-softmax-parity.sh [--snapshot-only|--live-only|--update-only]'; exit 0 ;;
...
if $LIVE; then
  cmake -S "$ROOT_DIR/tools/emel-kernel-target-softmax-reference" -B "$BUILD_DIR/cpp-build" -G Ninja -DEMEL_CPP_SOURCE_DIR="$EMEL_CPP_SOURCE" -DFETCHCONTENT_SOURCE_DIR_STATEFORWARD_SML="$SML_SOURCE" -DCMAKE_BUILD_TYPE=Release -DEMEL_ENABLE_TESTS=OFF -DEMEL_ENABLE_FUZZ=OFF >"$BUILD_DIR/logs/cmake-configure.log" 2>&1
  cmake --build "$BUILD_DIR/cpp-build" --target emel-kernel-target-softmax-reference >"$BUILD_DIR/logs/cmake-build.log" 2>&1
  cargo build --locked --offline -p emel-kernel-target-softmax-parity >"$BUILD_DIR/logs/cargo-build.log" 2>&1
  "$BUILD_DIR/cpp-build/emel-kernel-target-softmax-reference" >"$BUILD_DIR/cpp.out"
  "$BUILD_DIR/target/debug/emel-kernel-target-softmax-parity" >"$BUILD_DIR/rust.out"
fi
...
{ cat "$BUILD_DIR/rust.out"; printf 'result=live_split_observer_match\n'; printf 'reference_execution=pinned_aarch64_sm_process_event\n'; printf 'rust_execution=public_target_aarch64_kernel_process_soft_max\n'; printf 'source_aarch64_transition_span=src/emel/kernel/aarch64/sm.hpp:741-748\n'; printf 'source_detail_span=src/emel/kernel/detail.hpp:3734-3784,4217-4222\n'; printf 'rust_router=crates/emel-kernels/src/aarch64/sm.rs\n'; printf 'rust_child=crates/emel-kernels/src/any/reductions.rs\n'; printf 'allocation=zero_after_construction\n'; printf 'unsafe=none\n'; printf 'residual=soft_max_back\n'; printf 'live_artifacts=.artifacts/kernel-goal/target-softmax-live\n'; } >"$BUILD_DIR/manifest.txt"
if $UPDATE; then install -m 0644 "$BUILD_DIR/manifest.txt" "$SNAPSHOT"; fi
$COMPARE && diff -u "$SNAPSHOT" "$BUILD_DIR/manifest.txt"
printf '%s\n' 'Kernel target AArch64 softmax parity passed'
