...
grep -qx \
  "algorithm=reject_block_tensor owner=generation_guards proof=direct_shared_attention_and_shortconv_rejection_tests,lfm2_lifecycle_tests" \
  "$INVENTORY"
...
validate_proof_reference() {
  ...
  case "$proof" in
    ...
    all_supported_and_unknown_labels_test | plan_accessors)
      ...
      ;;
    direct_shared_attention_and_shortconv_rejection_tests)
      proof_file="$ROOT_DIR/crates/emel-model/src/generation/tests.rs"
      proof_pattern=reject_block_tensor_classifies_both_opposite_families
      ;;
    lfm2_lifecycle_tests)
      proof_file="$ROOT_DIR/crates/emel-model/src/lfm2/tests.rs"
      proof_pattern=rejects_wrong_architecture_geometry_and_missing_global_tensor
      ;;
    attention)
      proof_file="$ROOT_DIR/crates/emel-model/src/generation/tests.rs"
      proof_pattern=public_actor_builds_source_exact_attention_contract_and_fourteen_stage_audit
...
RUST_OBSERVER="${CARGO_TARGET_DIR:-$ROOT_DIR/target}/debug/examples/generation_observer"
[[ -x "$RUST_OBSERVER" ]] || { echo "error: missing Rust generation observer: $RUST_OBSERVER" >&2; exit 1; }
...
"$RUST_OBSERVER" --fixture "$EMEL_CPP_SOURCE/$LLAMA_RELATIVE" >"$BUILD_DIR/rust.fixture"
