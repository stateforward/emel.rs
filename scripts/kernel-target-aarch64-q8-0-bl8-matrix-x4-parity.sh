...
BUILD_DIR="${EMEL_TARGET_AARCH64_Q8_0_PACKED_BL8_MATRIX_X4_LIVE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/kernel-goal/target-aarch64-q8-0-bl8-matrix-x4-live}"
SNAPSHOT="${EMEL_TARGET_AARCH64_Q8_0_PACKED_BL8_MATRIX_X4_SNAPSHOT:-$ROOT_DIR/snapshots/parity/kernel-target-aarch64-q8-0-bl8-matrix-x4/manifest.txt}"
...
for argument in "$@"; do
  case "$argument" in
    ...
    --help|-h) printf '%s\n' 'usage: scripts/kernel-target-aarch64-q8-0-bl8-matrix-x4-parity.sh [--snapshot|--no-snapshot] [--update|--no-update] [--snapshot-only|--live-only|--update-only]'; exit 0 ;;
...
if $LIVE; then
  cmake -S "$ROOT_DIR/tools/emel-kernel-target-aarch64-q8-0-bl8-matrix-x4-reference" -B "$BUILD_DIR/cpp-build" -G Ninja \
  ...
  cmake --build "$BUILD_DIR/cpp-build" --target emel-kernel-target-aarch64-q8-0-bl8-matrix-x4-reference >"$BUILD_DIR/logs/cmake-build.log" 2>&1
  cargo run --locked --offline --manifest-path "$ROOT_DIR/tools/emel-kernel-target-aarch64-q8-0-bl8-matrix-x4-parity/Cargo.toml" --quiet \
    >"$BUILD_DIR/rust.out" 2>"$BUILD_DIR/logs/rust.stderr"
  "$BUILD_DIR/cpp-build/emel-kernel-target-aarch64-q8-0-bl8-matrix-x4-reference" >"$BUILD_DIR/cpp.out" 2>"$BUILD_DIR/logs/cpp.stderr"
else
...
grep -F 'case=invalid_shape status=reject error=InvalidShape output_bits=7fc00001,7fc00001,7fc00001,7fc00001,7fc00001,7fc00001,7fc00001,7fc00001,7fc00001,7fc00001,7fc00001,7fc00001,7fc00001,7fc00001,7fc00001,7fc00001' "$BUILD_DIR/rust.out" >/dev/null

{
  ...
  printf 'source_aarch64_guard_span=src/emel/kernel/aarch64/guards.hpp:35-72\nsource_aarch64_action_span=src/emel/kernel/aarch64/guards.hpp:444-452\nsource_aarch64_execution_span=src/emel/kernel/aarch64/actions.hpp:6584-6667\nsource_aarch64_transition_span=src/emel/kernel/aarch64/sm.hpp:372-373\nreference_execution=pinned_aarch64_sm_process_event\nrust_execution=public_target_aarch64_kernel_process_event\n'
  printf 'guards_blob=%s\nactions_blob=%s\nstate_machine_blob=%s\n' "$GUARDS_BLOB" "$ACTIONS_BLOB" "$SM_BLOB"
  printf 'output_sha256=%s\n' "$(shasum -a 256 "$BUILD_DIR/rust.out" | awk '{print $1}')"
  printf 'rust_module=crates/emel-kernels/src/aarch64/q8_0_packed.rs\nrust_router=crates/emel-kernels/src/aarch64/sm.rs\nrust_tests=crates/emel-kernels/tests/arch/target_aarch64_kernel.rs::router_dispatches_q8_0_packed_bl8_matrix_x4\n'
  printf 'scope=q8_0_x4_bl8_matrix_x4_lhs_rhs_f32_destination_m4_k32\npositive_cases=q8_0_packed_bl8_matrix_x4_m4_k32_exact_output_bits\nrejection_cases=zero_count_and_non_block_k_sentinel_preserving\nobserver_diff=empty byte-for-byte match\nallocation_contract=zero allocations after construction (existing focused actor test)\nunsafe_contract=none\ndispatch_contract=run_to_completion_public_target_router\nlive_artifacts=.artifacts/kernel-goal/target-aarch64-q8-0-bl8-matrix-x4-live\nresiduals=F16 neon, generic quantized mul_mat, and x86 live target\ntoolchain=rustc 1.98.0; cargo 1.98.0; pulp 0.22.3; stateforward-sml %s\nresult=match\n' "$SML_COMMIT"
...
printf 'AArch64 target q8_0 packed bl8 matrix_x4 live parity passed (emel.cpp %s)\n' "$SOURCE_COMMIT"
