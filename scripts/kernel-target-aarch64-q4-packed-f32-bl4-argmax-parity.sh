...
BUILD_DIR="${EMEL_TARGET_AARCH64_Q4_PACKED_F32_BL4_ARGMAX_LIVE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/kernel-goal/q4-packed-f32-bl4-argmax}"
SNAPSHOT="${EMEL_TARGET_AARCH64_Q4_PACKED_F32_BL4_ARGMAX_SNAPSHOT:-$ROOT_DIR/snapshots/parity/kernel-target-aarch64-q4-packed-f32-bl4-argmax/manifest.txt}"
...
for argument in "$@"; do
  case "$argument" in
    ...
    --help|-h) printf '%s\n' 'usage: scripts/kernel-target-aarch64-q4-packed-f32-bl4-argmax-parity.sh [--snapshot|--no-snapshot] [--update|--no-update] [--snapshot-only|--live-only|--update-only]'; exit 0 ;;
...
if $LIVE; then
  cmake -S "$ROOT_DIR/tools/emel-kernel-target-aarch64-q4-packed-f32-bl4-argmax-reference" -B "$BUILD_DIR/cpp-build" -G Ninja \
  ...
  cmake --build "$BUILD_DIR/cpp-build" --target emel-kernel-target-aarch64-q4-packed-f32-bl4-argmax-reference >"$BUILD_DIR/logs/cmake-build.log" 2>&1
  cargo run --locked --offline --manifest-path "$ROOT_DIR/tools/emel-kernel-target-aarch64-q4-packed-f32-bl4-argmax-parity/Cargo.toml" --quiet \
    >"$BUILD_DIR/rust.out" 2>"$BUILD_DIR/logs/rust.stderr"
  "$BUILD_DIR/cpp-build/emel-kernel-target-aarch64-q4-packed-f32-bl4-argmax-reference" >"$BUILD_DIR/cpp.out" 2>"$BUILD_DIR/logs/cpp.stderr"
else
  [[ -f "$BUILD_DIR/cpp.out" && -f "$BUILD_DIR/rust.out" ]]
fi
[[ ! -s "$BUILD_DIR/logs/rust.stderr" ]]
[[ ! -s "$BUILD_DIR/logs/cpp.stderr" ]]
diff -u "$BUILD_DIR/cpp.out" "$BUILD_DIR/rust.out" >"$BUILD_DIR/logs/observer-diff.log"
grep -F 'case=zero_count status=reject error=InvalidShape output_bits=7fc00001 index=-1' "$BUILD_DIR/rust.out" >/dev/null
grep -F 'case=k_zero status=reject error=InvalidShape output_bits=7fc00001 index=-1' "$BUILD_DIR/rust.out" >/dev/null
grep -F 'case=non_block_k status=reject error=InvalidShape output_bits=7fc00001 index=-1' "$BUILD_DIR/rust.out" >/dev/null

{
  ...
  printf 'source_aarch64_guard_span=src/emel/kernel/aarch64/actions.hpp:1769-1798\nsource_aarch64_argmax_action_span=src/emel/kernel/aarch64/actions.hpp:5593-5642\nsource_aarch64_transition_span=src/emel/kernel/aarch64/sm.hpp:472\nreference_execution=pinned_aarch64_sm_dispatch_op_mul_mat_argmax\nrust_execution=public_target_aarch64_kernel_process_event\n'
  printf 'guards_blob=%s\nactions_blob=%s\nstate_machine_blob=%s\n' "$GUARDS_BLOB" "$ACTIONS_BLOB" "$SM_BLOB"
  printf 'output_sha256=%s\n' "$(shasum -a 256 "$BUILD_DIR/rust.out" | awk '{print $1}')"
  printf 'rust_module=crates/emel-kernels/src/aarch64/q4_packed.rs\nrust_router=crates/emel-kernels/src/aarch64/sm.rs\nrust_tests=crates/emel-kernels/tests/arch/target_aarch64_kernel.rs::router_dispatches_q4_packed_f32_bl4_argmax\n'
  printf 'scope=q4_k_x8_bl4_lhs_dense_f32_rhs_argmax_destination_m5_k256\npositive_cases=q4_packed_f32_bl4_argmax_m5_k256_exact_output_bits_and_index\nrejection_cases=zero_count_k_zero_and_non_block_k_sentinel_preserving\nobserver_diff=empty byte-for-byte match\nallocation_contract=zero allocations after construction (focused actor test)\nunsafe_contract=none\ndispatch_contract=run_to_completion_public_target_router\nlive_artifacts=.artifacts/kernel-goal/q4-packed-f32-bl4-argmax\nresiduals=crate-parity, PR, bl8 argmax, prepared argmax, F16 neon, and x86\ntoolchain=rustc 1.98.0; cargo 1.98.0; pulp 0.22.3; stateforward-sml %s\nresult=match\n' "$SML_COMMIT"
...
printf 'AArch64 target q4 packed F32 bl4 argmax live parity passed (emel.cpp %s)\n' "$SOURCE_COMMIT"
