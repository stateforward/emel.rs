...
BUILD_DIR="${EMEL_TARGET_AARCH64_Q4_PACKED_F32_BL4_LIVE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/kernel-goal/q4-packed-f32-bl4}"
SNAPSHOT="${EMEL_TARGET_AARCH64_Q4_PACKED_F32_BL4_SNAPSHOT:-$ROOT_DIR/snapshots/parity/kernel-target-aarch64-q4-packed-f32-bl4/manifest.txt}"
...
for argument in "$@"; do
  case "$argument" in
    ...
    --help|-h) printf '%s\n' 'usage: scripts/kernel-target-aarch64-q4-packed-f32-bl4-parity.sh [--snapshot|--no-snapshot] [--update|--no-update] [--snapshot-only|--live-only|--update-only]'; exit 0 ;;
...
if $LIVE; then
  cmake -S "$ROOT_DIR/tools/emel-kernel-target-aarch64-q4-packed-f32-bl4-reference" -B "$BUILD_DIR/cpp-build" -G Ninja \
  ...
  cmake --build "$BUILD_DIR/cpp-build" --target emel-kernel-target-aarch64-q4-packed-f32-bl4-reference >"$BUILD_DIR/logs/cmake-build.log" 2>&1
  cargo run --locked --offline --manifest-path "$ROOT_DIR/tools/emel-kernel-target-aarch64-q4-packed-f32-bl4-parity/Cargo.toml" --quiet \
    >"$BUILD_DIR/rust.out" 2>"$BUILD_DIR/logs/rust.stderr"
  "$BUILD_DIR/cpp-build/emel-kernel-target-aarch64-q4-packed-f32-bl4-reference" >"$BUILD_DIR/cpp.out" 2>"$BUILD_DIR/logs/cpp.stderr"
else
...
printf 'AArch64 target q4 packed f32 bl4 live parity passed (emel.cpp %s)\n' "$SOURCE_COMMIT"
