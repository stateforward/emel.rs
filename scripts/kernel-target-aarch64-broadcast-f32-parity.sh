...
BUILD_DIR="${EMEL_TARGET_AARCH64_BROADCAST_F32_LIVE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/kernel-goal/target-aarch64-broadcast-f32-live}"
SNAPSHOT="${EMEL_TARGET_AARCH64_BROADCAST_F32_SNAPSHOT:-$ROOT_DIR/snapshots/parity/kernel-target-aarch64-broadcast-f32/manifest.txt}"
...
usage() {
  cat <<'USAGE'
usage: scripts/kernel-target-aarch64-broadcast-f32-parity.sh [OPTIONS]

Runs the pinned C++ and public Rust AArch64 broadcast observers. The default
also compares the generated manifest with the committed snapshot.
...
if $LIVE; then
  cmake -S "$ROOT_DIR/tools/emel-kernel-target-aarch64-broadcast-f32-reference" \
  ...
  cmake --build "$BUILD_DIR/cpp-build" \
    --target emel-kernel-target-aarch64-broadcast-f32-reference \
    >"$BUILD_DIR/logs/cmake-build.log" 2>&1
  cargo build --locked --offline -p emel-kernel-target-aarch64-broadcast-f32-parity \
    >"$BUILD_DIR/logs/cargo-build.log" 2>&1
  "$BUILD_DIR/cpp-build/emel-kernel-target-aarch64-broadcast-f32-reference" \
    >"$BUILD_DIR/cpp.out" 2>"$BUILD_DIR/logs/cpp.stderr"
  "$CARGO_TARGET_DIR/debug/emel-kernel-target-aarch64-broadcast-f32-parity" \
...

{
  ...
  printf 'source_detail_guard_span=src/emel/kernel/detail.hpp:3804-3819\n'
  printf 'source_detail_run_span=src/emel/kernel/detail.hpp:3822-3834\n'
  printf 'source_aarch64_guard_span=src/emel/kernel/aarch64/guards.hpp:1058-1080\n'
  printf 'source_aarch64_action_span=src/emel/kernel/aarch64/actions.hpp:9261-9268\n'
  printf 'source_aarch64_transition_span=src/emel/kernel/aarch64/sm.hpp:56-63,121-128\n'
  ...
  printf 'rust_module=crates/emel-kernels/src/aarch64/broadcast.rs\n'
  ...
  printf 'scope=dense_f32_row_broadcast_add_mul_neon_with_explicit_invalid_route\n'
  printf 'live_artifacts=.artifacts/kernel-goal/target-aarch64-broadcast-f32-live\n'
  printf 'positive_cases=row_add/row_mul exact output bits\n'
  ...
  printf 'residuals=non-dense tensor layouts; portable tensor-view dispatch; scalar/quantized/non-F32 routes; x86 live target unavailable on this host\n'
...
printf 'AArch64 target broadcast F32 live parity passed (emel.cpp %s)\n' "$SOURCE_COMMIT"
