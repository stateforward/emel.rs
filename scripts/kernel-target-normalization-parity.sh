...
BUILD_DIR="${EMEL_TARGET_NORMALIZATION_LIVE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/kernel-goal/target-normalization-live}"
SNAPSHOT="${EMEL_TARGET_NORMALIZATION_SNAPSHOT:-$ROOT_DIR/snapshots/parity/kernel-target-normalization/manifest.txt}"
...
for argument in "$@"; do
  case "$argument" in
    ...
    --help|-h) printf '%s\n' 'usage: scripts/kernel-target-normalization-parity.sh [--snapshot-only|--live-only|--update-only]'; exit 0 ;;
    *) printf 'error: unknown argument: %s\n' "$argument" >&2; exit 2 ;;
  esac
done
for command in cargo cmake diff git ninja rustc shasum; do command -v "$command" >/dev/null || { echo "error: required command is missing: $command" >&2; exit 2; }; done
...
if $LIVE; then
  cmake -S "$ROOT_DIR/tools/emel-kernel-target-normalization-reference" -B "$BUILD_DIR/cpp-build" -G Ninja -DEMEL_CPP_SOURCE_DIR="$EMEL_CPP_SOURCE" -DFETCHCONTENT_SOURCE_DIR_STATEFORWARD_SML="$SML_SOURCE" -DCMAKE_BUILD_TYPE=Release -DEMEL_ENABLE_TESTS=OFF -DEMEL_ENABLE_FUZZ=OFF >"$BUILD_DIR/logs/cmake-configure.log" 2>&1
  cmake --build "$BUILD_DIR/cpp-build" --target emel-kernel-target-normalization-reference >"$BUILD_DIR/logs/cmake-build.log" 2>&1
  cargo build --locked --offline -p emel-kernel-target-normalization-parity >"$BUILD_DIR/logs/cargo-build.log" 2>&1
  "$BUILD_DIR/cpp-build/emel-kernel-target-normalization-reference" >"$BUILD_DIR/cpp.out"
  "$BUILD_DIR/target/debug/emel-kernel-target-normalization-parity" >"$BUILD_DIR/rust.out"
fi
...
{ cat "$BUILD_DIR/rust.out"; printf 'result=live_split_observer_match\n'; printf 'reference_execution=pinned_aarch64_sm_process_event\n'; printf 'rust_execution=public_target_aarch64_kernel_process_normalization\n'; printf 'source_aarch64_transition_span=src/emel/kernel/aarch64/sm.hpp:306-323\n'; printf 'source_detail_span=src/emel/kernel/detail.hpp:4546-4648\n'; printf 'rust_router=crates/emel-kernels/src/aarch64/sm.rs\n'; printf 'rust_child=crates/emel-kernels/src/any/normalization.rs\n'; printf 'allocation=zero_after_construction\n'; printf 'unsafe=none\n'; printf 'live_artifacts=.artifacts/kernel-goal/target-normalization-live\n'; } >"$BUILD_DIR/manifest.txt"
if $UPDATE; then install -m 0644 "$BUILD_DIR/manifest.txt" "$SNAPSHOT"; fi
$COMPARE && diff -u "$SNAPSHOT" "$BUILD_DIR/manifest.txt"
printf '%s\n' 'Kernel target AArch64 normalization parity passed'
