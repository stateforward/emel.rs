...
MODE="run"
TARGETS=(gguf_loader gguf_load gguf_lifecycle gguf_metadata emel_io_read emel_io_mmap emel_io_staged_read emel_io_loader emel_model_tensor emel_model_catalog emel_model_generation emel_model_sortformer emel_model_omniembed emel_token_profile emel_model_vocabulary emel_tensor_dtype)
...
for target in "${TARGETS[@]}"; do
  if [[ "$target" != "emel_io_read" && "$target" != "emel_io_mmap" && "$target" != "emel_io_staged_read" && "$target" != "emel_io_loader" && "$target" != "emel_model_tensor" && "$target" != "emel_model_catalog" && "$target" != "emel_model_generation" && "$target" != "emel_model_sortformer" && "$target" != "emel_model_omniembed" && "$target" != "emel_token_profile" && "$target" != "emel_model_vocabulary" && "$target" != "emel_tensor_dtype" ]]; then
...
for target in "${TARGETS[@]}"; do
  if [[ "$MODE" == "build" ]]; then
    ...
    elif [[ "$target" == "emel_io_read" || "$target" == "emel_io_mmap" || "$target" == "emel_io_staged_read" || "$target" == "emel_io_loader" || "$target" == "emel_model_tensor" || "$target" == "emel_model_catalog" || "$target" == "emel_model_generation" || "$target" == "emel_model_sortformer" || "$target" == "emel_model_omniembed" || "$target" == "emel_token_profile" || "$target" == "emel_tensor_dtype" ]]; then
