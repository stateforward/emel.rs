...
if [[ -n "${EMEL_CPP_SOURCE_DIR:-}" ]]; then
  ...
  EMEL_CPP_SOURCE="$(find "$ROOT_DIR/../emel.cpp-worktrees" -maxdepth 1 -type d \
    -name 'pinned-843a117*' -print -quit)"
fi
SML_SOURCE="${EMEL_STATEFORWARD_SML_SOURCE:-$ROOT_DIR/../../sml/sml.cpp-worktrees/pinned-49207123cd3f39767764bae774932cb48623f92f}"
BUILD_DIR="${EMEL_TARGET_AARCH64_POWER_F32_LIVE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/kernel-goal/target-aarch64-power-f32-live}"
SNAPSHOT="${EMEL_TARGET_AARCH64_POWER_F32_SNAPSHOT:-$ROOT_DIR/snapshots/parity/kernel-target-aarch64-power-f32/manifest.txt}"
...
usage() {
  cat <<'USAGE'
usage: scripts/kernel-target-aarch64-power-f32-parity.sh [OPTIONS]

Runs pinned C++ and public Rust AArch64 op_sqr/op_sqrt observers. Snapshot,
...
for command in cargo cmake diff git ninja shasum; do
...
for entry in \
  ...
  "src/emel/kernel/aarch64/sm.hpp:$SM_BLOB"; do
  path="${entry%%:*}"
  expected="${entry##*:}"
...
if $LIVE; then
  cmake -S "$ROOT_DIR/tools/emel-kernel-target-aarch64-power-f32-reference" \
  ...
  cmake --build "$BUILD_DIR/cpp-build" \
    --target emel-kernel-target-aarch64-power-f32-reference \
    >"$BUILD_DIR/logs/cmake-build.log" 2>&1
  cargo build --locked --offline -p emel-kernel-target-aarch64-power-f32-parity \
    >"$BUILD_DIR/logs/cargo-build.log" 2>&1
  "$BUILD_DIR/cpp-build/emel-kernel-target-aarch64-power-f32-reference" \
    >"$BUILD_DIR/cpp.out" 2>"$BUILD_DIR/logs/cpp.stderr"
  "$CARGO_TARGET_DIR/debug/emel-kernel-target-aarch64-power-f32-parity" \
...
grep -F 'malformed_metadata status=reject output_preserved=true' \
  "$BUILD_DIR/logs/cpp.stderr" >"$BUILD_DIR/logs/malformed-metadata-probe.log"
...

{
  ...
  printf 'source_aarch64_guard_span=src/emel/kernel/aarch64/guards.hpp:805-824\n'
  printf 'source_aarch64_action_span=src/emel/kernel/aarch64/actions.hpp:886-930,2346-2400,8464-8496\n'
  printf 'source_aarch64_transition_span=src/emel/kernel/aarch64/sm.hpp:145-173\n'
  ...
  printf 'rust_module=crates/emel-kernels/src_aarch64_power_f32.rs\n'
  printf 'rust_router=crates/emel-kernels/src_aarch64_kernel.rs\n'
  printf 'rust_tests=crates/emel-kernels/tests/target_aarch64_power_f32.rs\n'
  printf 'scope=dense_f32_power_sqr_sqrt_neon_body_tail_zero_count_and_typed_rejection\n'
  printf 'live_artifacts=.artifacts/kernel-goal/target-aarch64-power-f32-live\n'
  printf 'reference_metadata_probe=malformed_stride_rejected_and_sentinel_preserved\n'
...
printf 'AArch64 target power F32 live parity passed (emel.cpp %s)\n' "$SOURCE_COMMIT"
