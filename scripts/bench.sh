...
EMEL_MODEL_VOCAB_SML_SOURCE="${EMEL_STATEFORWARD_SML_SOURCE:-$ROOT_DIR/../../sml/sml.cpp-worktrees/pinned-49207123cd3f39767764bae774932cb48623f92f}"
...
Suites: --suite=gguf, --suite=io-read, --suite=io-mmap, --suite=io-staged-read, --suite=io-loader, --suite=model-tensor, --suite=model-data, --suite=model-catalog, --suite=model-generation, --suite=model-vocabulary, --suite=token-profile, --suite=kernel, --suite=kernel-capability
...
if [[ "$SUITE" == "kernel" ]]; then
  kernel_args=()
  if $SNAPSHOT_MODE && $UPDATE; then
    kernel_args+=(--update)
  elif $SNAPSHOT_MODE; then
    kernel_args+=(--snapshot-only)
  else
    kernel_args+=(--live-only)
  fi
  exec "$ROOT_DIR/scripts/kernel-benchmark-suite.sh" "${kernel_args[@]}"
fi
...
if [[ -n "${EMEL_BENCH_RUNNER:-}" ]]; then
  ...
  RUNNER="${CARGO_TARGET_DIR:-$ROOT_DIR/target}/release/emel-bench"
fi
mkdir -p "$BUILD_DIR"
