...
BUILD_DIR="${EMEL_MODEL_QWEN3_BENCH_BUILD_DIR:-$ROOT_DIR/target/model-qwen3-bench}"
...
SNAPSHOT=false
UPDATE=false
MAX_RATIO="${EMEL_QWEN3_BENCH_MAX_REGRESSION_RATIO:-2.0}"
...
EMEL_CPP_SOURCE_DIR="$SOURCE_DIR" \
  EMEL_MODEL_QWEN3_PARITY_BUILD_DIR="$BUILD_DIR/reference" \
  "$ROOT_DIR/scripts/model-qwen3-parity.sh" --live >/dev/null
...
while [[ "$process_run" -lt "$PROCESS_RUNS" ]]; do
  ...
  "${RUNNER[@]}" "$ROOT_DIR/target/release/examples/qwen3_observer" --benchmark "$ITERATIONS" "$RUNS" "$WARMUP" >>"$BUILD_DIR/rust.out"
...
paste "$BUILD_DIR/cpp.out" "$BUILD_DIR/rust.out" | awk -F'[ =\t]+' '
  {
    ...
    printf "model/qwen3/block_visit/process run=%d cpp_ns_per_op=%s rust_ns_per_op=%s paired_ratio=%.6f\n", NR, cpp, rust, rust / cpp
  }
' >"$SAMPLES"
...
awk -v ratio="$ratio" -v maximum="$MAX_RATIO" 'BEGIN { exit !(ratio <= maximum) }' || {
  echo "error: model Qwen3 Rust/C++ ratio $ratio exceeds $MAX_RATIO" >&2
  exit 1
}
...
BASELINE="$ROOT_DIR/snapshots/bench/model-qwen3-$target_os-$arch-$SCHEDULING_KEY.txt"
{
  printf '# source_commit: 843a117386ef17dc5a50549bbfc821074c2141d6\n'
  printf '# source_qwen3_detail_header_blob: 6e31b097184042bfe1ec9486af060214f1d01b2b\n'
  printf '# source_qwen3_detail_implementation_blob: a43d8fee3ed962d84447360e7601c999468bf6f8\n'
  printf '# benchmark_operand: same alternating completed Qwen3 headwise-QK-RMS attention block indices 0 and 1; Rust public family BlockVisit vs C++ public generation lookup_block_view over the source-built Qwen3 contract; full descriptor black-boxed and nonzero index checksum observed\n'
  ...
  cat "$SAMPLES"
  printf 'model/qwen3/block_visit rust_median_ns_per_op=%s cpp_median_ns_per_op=%s paired_ratio_median=%s outcome=found checksum=%s\n' "$rust" "$cpp" "$ratio" "$rust_checksum"
} >"$CURRENT"
...
if $SNAPSHOT; then
  [[ -f "$BASELINE" ]] || { echo "error: missing model Qwen3 benchmark baseline: $BASELINE" >&2; exit 1; }
  awk -v max_ratio="$MAX_RATIO" '
    ...
    FNR == NR {
      ...
      if ($1 == "model/qwen3/block_visit") baseline_rust = field("rust_median_ns_per_op")
      next
    }
    {
