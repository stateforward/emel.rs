...
BUILD_DIR="${EMEL_TARGET_AARCH64_Q4_K_MUL_MAT_LIVE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/kernel-goal/target-aarch64-q4-k-mul-mat-live}"
SNAPSHOT="${EMEL_TARGET_AARCH64_Q4_K_MUL_MAT_SNAPSHOT:-$ROOT_DIR/snapshots/parity/kernel-target-aarch64-q4-k-mul-mat/manifest.txt}"
...
for argument in "$@"; do
  case "$argument" in
    ...
    --help|-h) printf '%s\n' 'usage: scripts/kernel-target-aarch64-q4-k-mul-mat-parity.sh [--snapshot|--no-snapshot] [--update|--no-update] [--snapshot-only|--live-only|--update-only]'; exit 0 ;;
...
if $LIVE; then
  cmake -S "$ROOT_DIR/tools/emel-kernel-target-aarch64-q4-k-mul-mat-reference" -B "$BUILD_DIR/cpp-build" -G Ninja \
  ...
  cmake --build "$BUILD_DIR/cpp-build" --target emel-kernel-target-aarch64-q4-k-mul-mat-reference >"$BUILD_DIR/logs/cmake-build.log" 2>&1
  cargo run --locked --offline --manifest-path "$ROOT_DIR/tools/emel-kernel-target-aarch64-q4-k-mul-mat-parity/Cargo.toml" --quiet \
    >"$BUILD_DIR/rust.out" 2>"$BUILD_DIR/logs/rust.stderr"
  "$BUILD_DIR/cpp-build/emel-kernel-target-aarch64-q4-k-mul-mat-reference" >"$BUILD_DIR/cpp.out" 2>"$BUILD_DIR/logs/cpp.stderr"
else
...

{
  ...
  printf 'rust_module=crates/emel-kernels/src/aarch64/q4_k.rs\nrust_router=crates/emel-kernels/src/aarch64/sm.rs\nrust_tests=crates/emel-kernels/tests/arch/target_aarch64_kernel.rs::router_dispatches_q4_k_gemm\n'
  printf 'scope=q4_k_gemm_m2_k256_n2\npositive_cases=q4_k_mul_mat_m2_k256_n2_exact_output_bits\nrejection_cases=zero_count_and_non_block_k_sentinel_preserving\nobserver_diff=empty byte-for-byte match\nallocation_contract=zero allocations after construction (existing focused actor test)\nunsafe_contract=none\ndispatch_contract=run_to_completion_public_target_router\nlive_artifacts=.artifacts/kernel-goal/target-aarch64-q4-k-mul-mat-live\nresiduals=F16 neon, remaining quantized generic mul_mat (q2/q3), and x86 live target\ntoolchain=rustc 1.98.0; cargo 1.98.0; pulp 0.22.3; stateforward-sml %s\nresult=match\n' "$SML_COMMIT"
...
printf 'AArch64 target q4_k mul_mat live parity passed (emel.cpp %s)\n' "$SOURCE_COMMIT"
