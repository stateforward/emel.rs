//! Split Rust observer for the public AArch64 target power router.

#![cfg_attr(not(target_arch = "aarch64"), allow(dead_code))]

use emel_kernels::aarch64::{Kernel, PowerF32Error, PowerSqr, PowerSqrt};
...
fn main() {
    eprintln!("AArch64 power observer requires an AArch64 build");
    std::process::exit(2);
}
...
fn main() {
    ...
    println!("kernel-target-aarch64-power-f32-live/v1");
    ...
    println!("scope=target_router_power_f32_sqr_sqrt_dense_body_tail_and_rejection");
    ...
    assert_eq!(
        actor.process_event(PowerSqr::new(&input), &mut output),
        Ok(())
    );
    println!("case=sqr status=ok output_bits={}", bits(&output));
    assert_eq!(
        actor.process_event(PowerSqrt::new(&input), &mut output),
        Ok(())
    );
    println!("case=sqrt status=ok output_bits={}", bits(&output));

    let mut zero_output = [];
    assert_eq!(
        actor.process_event(PowerSqr::new(&[]), &mut zero_output),
        Err(PowerF32Error::InvalidShape)
    );
    println!("case=zero_sqr status=reject error=InvalidShape output_bits=");
    assert_eq!(
        actor.process_event(PowerSqrt::new(&[]), &mut zero_output),
        Err(PowerF32Error::InvalidShape)
    );
    println!("case=zero_sqrt status=reject error=InvalidShape output_bits=");

    let mut invalid_output = [SENTINEL; 2];
    assert_eq!(
        actor.process_event(PowerSqr::new(&[1.0, 4.0, 9.0]), &mut invalid_output),
        Err(PowerF32Error::InvalidShape)
    );
    assert!(
        invalid_output
