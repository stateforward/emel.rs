...
#error "q6 prepared matrix_x4 observer requires the live i8mm path"
#endif
...
constexpr std::size_t q6_prepared_block_bytes =
    sizeof(::emel::kernel::detail::quant::block_q6_kx8_q8_prepared);
...
constexpr std::size_t group_bytes = q6_prepared_block_bytes;
...
int main() {
  ...
  std::array<::emel::kernel::detail::quant::block_q6_k, m> src_rows{};
  for (std::uint64_t row = 0; row < m; ++row) {
    src_rows[row].d = static_cast<std::uint16_t>(0x3c00u + row * 17u);
    for (std::size_t index = 0; index < src_rows[row].ql.size(); ++index) {
      src_rows[row].ql[index] = static_cast<std::uint8_t>(
          (row * 29u + index * 5u + 3u) & 0xffu);
    }
    for (std::size_t index = 0; index < src_rows[row].qh.size(); ++index) {
      src_rows[row].qh[index] = static_cast<std::uint8_t>(
          (row * 19u + index * 7u + 11u) & 0xffu);
    }
    for (std::size_t index = 0; index < src_rows[row].scales.size(); ++index) {
      src_rows[row].scales[index] = static_cast<std::int8_t>(
          (static_cast<int>(row) * 3 + static_cast<int>(index)) % 15 - 7);
    }
  }
  std::array<std::uint8_t, group_count * q6_prepared_block_bytes> lhs{};
  if (!::emel::kernel::detail::quant::pack_q6_k_rows_x8_q8_prepared(
          src_rows.data(), m, k, lhs.data())) {
    return 1;
  }
  ...
  std::cout << "kernel-target-aarch64-q6-prepared-matrix-x4-live/v1\nsource_repository=stateforward/emel.cpp\nsource_commit="
            ...
            << "\ntarget_arch=aarch64\nscope=target_router_q6_prepared_matrix_x4_m8_k256_and_typed_rejection\nexecution=split_pinned_aarch64_sm_and_public_target_router\nlive_path=execute_neon_mul_mat_q6_vector_prepared_q8_rhs_i8mm_matrix_x4\n";

  emel::kernel::aarch64::sm kernel;
  std::array<float, rhs_rows * m> output{};
  emel::kernel::event::op_mul_mat request{};
  request.src0 = view(lhs.data(), emel::kernel::event::dtype::q6_k_x8_q8_prepared,
                      lhs_ne, lhs_nb);
  request.src1 = view(rhs.data(), emel::kernel::event::dtype::q8_k_x4, rhs_ne, rhs_nb);
  request.dst = view_mut(output.data(), dst_ne, dst_nb);
  if (!kernel.process_event(request)) return 1;
  if (kernel.optimized_q6_vector_prepared_q8_rhs_i8mm_dispatch_count() != 1u) {
    return 1;
  }
  std::cout << "case=q6_prepared_matrix_x4_m8_k256 status=ok output_bits=";
  write_bits(output);
  std::cout << '\n';
  ...
  invalid.src0 =
      view(zero.data(), emel::kernel::event::dtype::q6_k_x8_q8_prepared,
           {0, 0, 1, 1}, {1, 0, 0, 0});
  invalid.src1 = view(zero.data(), emel::kernel::event::dtype::q8_k_x4, {0, 0, 1, 1},
                      {1, 0, 0, 0});
  ...
  invalid.src0 = view(lhs.data(), emel::kernel::event::dtype::q6_k_x8_q8_prepared,
                      {0, m, 1, 1}, {1, 0, 0, 0});
  invalid.src1 = view(rhs.data(), emel::kernel::event::dtype::q8_k_x4, {rhs_rows, 0, 1, 1},
                      {1, 0, 0, 0});
  ...
  invalid.src0 = view(lhs.data(), emel::kernel::event::dtype::q6_k_x8_q8_prepared,
                      lhs_ne, lhs_nb);
  invalid.src1 =
      view(rhs.data(), emel::kernel::event::dtype::q8_k_x4, bad_rhs_ne, bad_rhs_nb);
