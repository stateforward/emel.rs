...
#error "q6 prepared matrix_x8 observer requires the live i8mm path"
#endif
...
int main() {
  ...
  std::cout << "kernel-target-aarch64-q6-prepared-matrix-x8-live/v1\nsource_repository=stateforward/emel.cpp\nsource_commit="
            ...
            << "\ntarget_arch=aarch64\nscope=target_router_q6_prepared_matrix_x8_m8_k256_and_typed_rejection\nexecution=split_pinned_aarch64_sm_and_public_target_router\nlive_path=execute_neon_mul_mat_q6_vector_prepared_q8_rhs_i8mm_matrix_x8\n";
  ...
  std::cout << "case=q6_prepared_matrix_x8_m8_k256 status=ok output_bits=";
  write_bits(output);
  std::cout << '\n';

  std::array<float, 8> zero;
  zero.fill(k_sentinel);
  ...
  invalid.src1 = view(zero.data(), emel::kernel::event::dtype::q8_k_x8, {0, 0, 1, 1},
                      {1, 0, 0, 0});
  invalid.dst = view_mut(zero.data(), {8, 0, 1, 1}, {0, 4, 0, 0});
