//! Split Rust observer for the public AArch64 q6 packed x8/q8-rhs router.
...
use emel_kernels::aarch64::{Kernel, MulMatQ6Packed, Q6PackedError};
...
fn main() {
    eprintln!("AArch64 q6 packed observer requires an AArch64 build");
    std::process::exit(2);
}
...
#[cfg(target_arch = "aarch64")]
fn main() {
    println!("kernel-target-aarch64-q6-packed-live/v1");
    ...
    println!("scope=target_router_q6_packed_m5_k256_and_typed_rejection");
    ...
    assert_eq!(
        actor.process_event(MulMatQ6Packed::new(&lhs, &rhs, M, K), &mut output),
        Ok(())
    );
    println!(
        "case=q6_packed_m5_k256 status=ok output_bits={}",
        bits(&output)
    );

    let mut zero_output = [SENTINEL];
    assert_eq!(
        actor.process_event(MulMatQ6Packed::new(&[], &[], 0, 0), &mut zero_output),
    ...
    assert_eq!(
        actor.process_event(MulMatQ6Packed::new(&lhs, &rhs, M, 255), &mut invalid_output),
        Err(Q6PackedError::InvalidShape)
    );
    println!(
