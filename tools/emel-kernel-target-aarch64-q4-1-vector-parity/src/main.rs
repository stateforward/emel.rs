//! Split Rust observer for the public AArch64 q4_1 vector router.

#![cfg_attr(not(target_arch = "aarch64"), allow(dead_code))]

use emel_kernels::aarch64::{Kernel, MulMatQ4_1Vector, Q4_1VectorError};
...
fn main() {
    eprintln!("AArch64 q4_1 vector observer requires an AArch64 build");
    std::process::exit(2);
}
...
#[cfg(target_arch = "aarch64")]
fn pack_lhs() -> [u8; M * Q4_1_BLOCK_BYTES] {
    let mut lhs = [0_u8; M * Q4_1_BLOCK_BYTES];
    for row in 0..M {
        let offset = row * Q4_1_BLOCK_BYTES;
        lhs[offset..offset + 2].copy_from_slice(&0x3c00_u16.to_le_bytes());
        lhs[offset + 2..offset + 4].copy_from_slice(&0x3800_u16.to_le_bytes());
        for index in 0..16 {
            let low = u8::try_from((row + index) % 16).unwrap();
            let high = u8::try_from((row * 3 + index) % 16).unwrap();
            lhs[offset + 4 + index] = low | (high << 4);
        }
    }
    lhs
}
...
#[cfg(target_arch = "aarch64")]
fn main() {
    println!("kernel-target-aarch64-q4-1-vector-live/v1");
    ...
    println!("scope=target_router_q4_1_vector_m5_k32_and_typed_rejection");
    ...
    assert_eq!(
        actor.process_event(MulMatQ4_1Vector::new(&lhs, &rhs, M, K), &mut output),
        Ok(())
    );
    println!(
        "case=q4_1_vector_m5_k32 status=ok output_bits={}",
        bits(&output)
    );

    let mut zero_output = [SENTINEL];
    assert_eq!(
        actor.process_event(MulMatQ4_1Vector::new(&[], &[], 0, 0), &mut zero_output),
    ...
    assert_eq!(
        actor.process_event(
            MulMatQ4_1Vector::new(&lhs, &rhs, M, 31),
            &mut invalid_output
        ),
        Err(Q4_1VectorError::InvalidShape)
    );
