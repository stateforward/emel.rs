...
constexpr std::size_t q2_bytes = 84;
...
void pack_q2(::emel::kernel::detail::quant::block_q2_k &block, std::uint16_t d,
             std::uint16_t dmin, std::uint8_t scales, std::uint8_t qs) {
...
int main() {
  std::array<::emel::kernel::detail::quant::block_q2_k, m> lhs_blocks{};
  pack_q2(lhs_blocks[0], 0x3c00, 0x3800, 0x12, 0xe4);
  pack_q2(lhs_blocks[1], 0x4000, 0x3400, 0x34, 0x1b);
  ...
  constexpr std::array<std::uint64_t, 4> lhs_nb = {1, q2_bytes, q2_bytes * m, q2_bytes * m};
  ...
  std::cout << "kernel-target-aarch64-q2-k-mul-mat-live/v1\nsource_repository=stateforward/emel.cpp\nsource_commit="
  ...
  std::cout << "case=q2_k_mul_mat_m2_k256_n2 status=ok output_bits=";
  write_bits(output);
  std::cout << '\n';

  std::array<float, 4> zero = {k_sentinel, k_sentinel, k_sentinel, k_sentinel};
  emel::kernel::event::op_mul_mat invalid{};
  invalid.src0 = view(zero.data(), emel::kernel::event::dtype::q2_k, {0, 0, 1, 1}, {1, 0, 0, 0});
  ...
  invalid.src0 = view(lhs_blocks.data(), emel::kernel::event::dtype::q2_k, {0, m, 1, 1},
                      {1, q2_bytes, q2_bytes * m, q2_bytes * m});
