//! Split Rust observer for the public AArch64 q4 packed x8/bl4 F32-rhs argmax router.
...
use emel_kernels::aarch64::{Kernel, MulMatArgmaxQ4PackedF32Bl4, Q4PackedBl4Error};
...
fn main() {
    eprintln!("AArch64 q4 packed f32 bl4 observer requires an AArch64 build");
    std::process::exit(2);
}
...
fn make_block_q4_k_x8_bl4(
    ...
    for index in 0..end {
        ...
        out[128 + dst_offset..128 + dst_offset + INTERLEAVE]
            .copy_from_slice(&rows[src_row][16 + src_offset..20 + src_offset]);
    }
...
#[cfg(target_arch = "aarch64")]
fn main() {
    println!("kernel-target-aarch64-q4-packed-f32-bl4-argmax-live/v1");
    ...
    println!("scope=target_router_q4_packed_f32_bl4_argmax_m5_k256_and_typed_rejection");
    println!("execution=split_pinned_aarch64_sm_dispatch_and_public_target_router");
    ...
    let index = actor
        .process_event(
            MulMatArgmaxQ4PackedF32Bl4::new(&lhs, &rhs, M, K),
            &mut output,
        )
        .expect("q4 packed F32 bl4 argmax");
    println!(
        "case=q4_packed_f32_bl4_argmax_m5_k256 status=ok output_bits={} index={}",
        bits(&output),
        index
    );

    let mut zero_output = [SENTINEL];
    let zero_index = INDEX_SENTINEL;
    assert_eq!(
        actor.process_event(
            MulMatArgmaxQ4PackedF32Bl4::new(&[], &[], 0, 0),
            &mut zero_output,
        ),
    ...
    assert_eq!(
        actor.process_event(
            MulMatArgmaxQ4PackedF32Bl4::new(&[], &[], M, 0),
            &mut invalid_output
        ),
        Err(Q4PackedBl4Error::InvalidShape)
    );
    println!(
        "case=k_zero status=reject error=InvalidShape output_bits={} index={}",
        bits(&invalid_output),
        INDEX_SENTINEL
    );

    let mut non_block_output = [SENTINEL];
    assert_eq!(
        actor.process_event(
            MulMatArgmaxQ4PackedF32Bl4::new(&[0_u8; 1], &[0.0_f32; 255], M, 255),
            &mut non_block_output,
        ),
        Err(Q4PackedBl4Error::InvalidShape)
    );
    println!(
        "case=non_block_k status=reject error=InvalidShape output_bits={} index={}",
        bits(&non_block_output),
        INDEX_SENTINEL
    );
}
