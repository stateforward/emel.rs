//! Split Rust observer for the public AArch64 q4 packed x8/bl4 F32-rhs router.
...
use emel_kernels::aarch64::{Kernel, MulMatQ4PackedF32Bl4, Q4PackedBl4Error};
...
#[cfg(target_arch = "aarch64")]
fn main() {
    println!("kernel-target-aarch64-q4-packed-f32-bl4-live/v1");
    ...
    println!("scope=target_router_q4_packed_f32_bl4_m5_k256_and_typed_rejection");
    ...
    assert_eq!(
        actor.process_event(MulMatQ4PackedF32Bl4::new(&lhs, &rhs, M, K), &mut output),
        Ok(())
    );
    println!(
        "case=q4_packed_f32_bl4_m5_k256 status=ok output_bits={}",
        bits(&output)
    );

    let mut zero_output = [SENTINEL];
    assert_eq!(
        actor.process_event(MulMatQ4PackedF32Bl4::new(&[], &[], 0, 0), &mut zero_output),
    ...
    assert_eq!(
        actor.process_event(
            MulMatQ4PackedF32Bl4::new(&lhs, &rhs, M, 0),
            &mut invalid_output
        ),
        Err(Q4PackedBl4Error::InvalidShape)
    );
