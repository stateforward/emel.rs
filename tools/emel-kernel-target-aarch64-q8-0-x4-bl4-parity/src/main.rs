//! Split Rust observer for the public AArch64 q8_0 packed x4/bl4 router.

#![cfg_attr(not(target_arch = "aarch64"), allow(dead_code))]

use emel_kernels::aarch64::{Kernel, MulMatQ8_0PackedBl4, Q8_0PackedBl4Error};
...
fn main() {
    eprintln!("AArch64 q8_0 packed bl4 observer requires an AArch64 build");
    std::process::exit(2);
}
...
#[cfg(target_arch = "aarch64")]
fn main() {
    println!("kernel-target-aarch64-q8-0-x4-bl4-live/v1");
    ...
    println!("scope=target_router_q8_0_packed_bl4_m5_k32_and_typed_rejection");
    ...
    assert_eq!(
        actor.process_event(MulMatQ8_0PackedBl4::new(&lhs, &rhs, M, K), &mut output),
        Ok(())
    );
    println!(
        "case=q8_0_packed_bl4_m5_k32 status=ok output_bits={}",
        bits(&output)
    );

    let mut zero_output = [SENTINEL];
    assert_eq!(
        actor.process_event(MulMatQ8_0PackedBl4::new(&[], &[], 0, 0), &mut zero_output),
    ...
    assert_eq!(
        actor.process_event(
            MulMatQ8_0PackedBl4::new(&lhs, &rhs, M, 31),
            &mut invalid_output
        ),
        Err(Q8_0PackedBl4Error::InvalidShape)
    );
