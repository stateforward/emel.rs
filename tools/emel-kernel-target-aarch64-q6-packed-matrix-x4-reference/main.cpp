...
constexpr std::size_t q6_x8_block_bytes =
    sizeof(::emel::kernel::detail::quant::block_q6_kx8);
...
constexpr std::size_t group_bytes = q6_x8_block_bytes;
...
int main() {
  ...
  std::array<std::uint8_t, group_count * q6_x8_block_bytes> lhs{};
  if (!::emel::kernel::detail::quant::pack_q6_k_rows_x8(src_rows.data(), m, k,
                                                       lhs.data())) {
    return 1;
  }
  ...
  std::cout << "kernel-target-aarch64-q6-packed-matrix-x4-live/v1\nsource_repository=stateforward/emel.cpp\nsource_commit="
  ...
  std::cout << "case=q6_packed_matrix_x4_m8_k256 status=ok output_bits=";
  write_bits(output);
  std::cout << '\n';

  std::array<float, 4> zero = {k_sentinel, k_sentinel, k_sentinel, k_sentinel};
  emel::kernel::event::op_mul_mat invalid{};
  invalid.src0 = view(zero.data(), emel::kernel::event::dtype::q6_k_x8,
                      {0, 0, 1, 1}, {1, 0, 0, 0});
  ...
  invalid.src0 = view(lhs.data(), emel::kernel::event::dtype::q6_k_x8,
                      {0, m, 1, 1}, {1, 0, 0, 0});
