//! Split Rust observer for the public AArch64 q4_0 vector router.

#![cfg_attr(not(target_arch = "aarch64"), allow(dead_code))]

use emel_kernels::aarch64::{Kernel, MulMatQ4_0Vector, Q4_0VectorError};
...
fn main() {
    eprintln!("AArch64 q4_0 vector observer requires an AArch64 build");
    std::process::exit(2);
}
...
#[cfg(target_arch = "aarch64")]
fn pack_lhs() -> [u8; M * Q4_0_BLOCK_BYTES] {
    let mut lhs = [0_u8; M * Q4_0_BLOCK_BYTES];
    for row in 0..M {
        let offset = row * Q4_0_BLOCK_BYTES;
...
#[cfg(target_arch = "aarch64")]
fn main() {
    println!("kernel-target-aarch64-q4-0-vector-live/v1");
    ...
    println!("scope=target_router_q4_0_vector_m5_k32_and_typed_rejection");
    ...
    assert_eq!(
        actor.process_event(MulMatQ4_0Vector::new(&lhs, &rhs, M, K), &mut output),
        Ok(())
    );
    println!(
        "case=q4_0_vector_m5_k32 status=ok output_bits={}",
        bits(&output)
    );

    let mut zero_output = [SENTINEL];
    assert_eq!(
        actor.process_event(MulMatQ4_0Vector::new(&[], &[], 0, 0), &mut zero_output),
    ...
    assert_eq!(
        actor.process_event(
            MulMatQ4_0Vector::new(&lhs, &rhs, M, 31),
            &mut invalid_output
        ),
        Err(Q4_0VectorError::InvalidShape)
    );
