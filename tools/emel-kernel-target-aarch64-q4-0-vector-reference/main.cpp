...
constexpr std::size_t q4_block_bytes = 18;
...
int main() {
  ...
  for (std::uint64_t row = 0; row < m; ++row) {
    ...
    for (std::size_t index = 0; index < 16; ++index) {
      ...
      lhs[offset + 2 + index] = static_cast<std::uint8_t>(low | (high << 4));
    }
  }
  ...
  std::cout << "kernel-target-aarch64-q4-0-vector-live/v1\nsource_repository=stateforward/emel.cpp\nsource_commit="
  ...
  std::cout << "case=q4_0_vector_m5_k32 status=ok output_bits=";
  write_bits(output);
  std::cout << '\n';

  std::array<float, 1> zero = {k_sentinel};
  emel::kernel::event::op_mul_mat invalid{};
  invalid.src0 = view(zero.data(), emel::kernel::event::dtype::q4_0, {0, 0, 1, 1}, {1, 0, 0, 0});
  ...
  invalid.src0 = view(lhs.data(), emel::kernel::event::dtype::q4_0, {31, m, 1, 1},
