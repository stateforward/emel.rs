...
int main() {
  ...
  constexpr std::array<std::uint64_t, 4> lhs_nb = {1, q8_block_bytes, q8_block_bytes * m,
                                                   q8_block_bytes * m};
  ...
  std::array<std::uint8_t, m * q8_block_bytes> lhs{};
  for (std::uint64_t row = 0; row < m; ++row) {
    const std::size_t offset = static_cast<std::size_t>(row) * q8_block_bytes;
    lhs[offset] = 0x00;
    lhs[offset + 1] = 0x3c;
    for (std::size_t index = 0; index < 32; ++index) {
      const int value = static_cast<int>(index) - 16 + static_cast<int>(row);
      lhs[offset + 2 + index] = static_cast<std::uint8_t>(static_cast<std::int8_t>(value));
    }
  }
  ...
  rhs[1] = 0x38;
  for (std::size_t index = 0; index < 32; ++index) {
    const int value = static_cast<int>(index) - 16;
    rhs[2 + index] = static_cast<std::uint8_t>(static_cast<std::int8_t>(value));
  }

  std::cout << "kernel-target-aarch64-q8-0-q8-rhs-live/v1\nsource_repository=stateforward/emel.cpp\nsource_commit="
  ...
  std::cout << "case=q8_0_q8_rhs_m5_k32 status=ok output_bits=";
  write_bits(output);
  std::cout << '\n';

  std::array<float, 1> zero = {k_sentinel};
  emel::kernel::event::op_mul_mat invalid{};
  invalid.src0 = view(zero.data(), emel::kernel::event::dtype::q8_0, {0, 0, 1, 1}, {1, 0, 0, 0});
  ...
  invalid.src0 = view(lhs.data(), emel::kernel::event::dtype::q8_0, {31, m, 1, 1},
                      {1, q8_block_bytes, q8_block_bytes * m, q8_block_bytes * m});
