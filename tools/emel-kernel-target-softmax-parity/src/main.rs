//! Split Rust observer for the public AArch64 target softmax router.

use emel_kernels::aarch64::{Kernel, SoftMaxEvent, UnexpectedSoftMax};
...
fn main() {
    eprintln!("AArch64 softmax observer requires an AArch64 build");
    std::process::exit(2);
}
...
fn main() {
    ...
    println!("kernel-target-softmax-live/v1");
    ...
    println!("scope=target_router_soft_max_f32_dense");
    ...
    actor
        .process_soft_max(OpSoftMax::new(
            TensorView::new(&input, layout),
            TensorViewMut::new(&mut output, layout),
        ))
        .expect("softmax");
    println!("case=soft_max status=ok output_bits={}", bits(&output));

    let zero_layout = Layout::new(DType::F32, [0, 2, 1, 1], [4, 0, 0, 0]);
    let mut zero_output = [SENTINEL; 1];
    assert_eq!(
        actor.process_soft_max(OpSoftMax::new(
            TensorView::new(&[SENTINEL], zero_layout),
            TensorViewMut::new(&mut zero_output, zero_layout),
        )),
    ...
    assert_eq!(
        SoftMaxEvent::dispatch(UnexpectedSoftMax, &mut actor),
