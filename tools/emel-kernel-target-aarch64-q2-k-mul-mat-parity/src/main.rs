//! Split Rust observer for the public AArch64 q2_k GEMM router.

#![cfg_attr(not(target_arch = "aarch64"), allow(dead_code))]

use emel_kernels::aarch64::{Kernel, MulMatQ2K, Q2KError};
...
fn main() {
    eprintln!("AArch64 q2_k mul_mat observer requires an AArch64 build");
    std::process::exit(2);
}
...
#[cfg(target_arch = "aarch64")]
fn pack_q2_row(d: u16, dmin: u16, scale: u8, qs: u8) -> [u8; Q2_K_BLOCK_BYTES] {
...
#[cfg(target_arch = "aarch64")]
fn main() {
    println!("kernel-target-aarch64-q2-k-mul-mat-live/v1");
    ...
    println!("scope=target_router_q2_k_mul_mat_m2_k256_n2_and_typed_rejection");
    println!("execution=split_pinned_aarch64_sm_and_public_target_router");

    let row0 = pack_q2_row(0x3c00, 0x3800, 0x12, 0xe4);
    let row1 = pack_q2_row(0x4000, 0x3400, 0x34, 0x1b);
    ...
    assert_eq!(
        actor.process_event(MulMatQ2K::new(&lhs, &rhs, M, QK_K, N), &mut output),
        Ok(())
    );
    println!(
        "case=q2_k_mul_mat_m2_k256_n2 status=ok output_bits={}",
        bits(&output)
    );

    let mut zero_output = [SENTINEL; 4];
    assert_eq!(
        actor.process_event(MulMatQ2K::new(&[], &[], 0, 0, 0), &mut zero_output),
    ...
    assert_eq!(
        actor.process_event(MulMatQ2K::new(&lhs, &rhs, M, 0, N), &mut invalid_output),
        Err(Q2KError::InvalidShape)
    );
    println!(
