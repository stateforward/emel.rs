...
constexpr std::size_t q5_block_bytes = 22;
...
int main() {
  ...
  constexpr std::array<std::uint64_t, 4> lhs_nb = {1, q5_block_bytes, q5_block_bytes * m,
                                                   q5_block_bytes * m};
  ...
  std::array<std::uint8_t, m * q5_block_bytes> lhs{};
  for (std::uint64_t row = 0; row < m; ++row) {
    const std::size_t offset = static_cast<std::size_t>(row) * q5_block_bytes;
    lhs[offset] = 0x00;
    lhs[offset + 1] = 0x3c;
    const std::uint32_t qh = 0xa5a55a5au ^ static_cast<std::uint32_t>(row * 17u);
    lhs[offset + 2] = static_cast<std::uint8_t>(qh);
    lhs[offset + 3] = static_cast<std::uint8_t>(qh >> 8);
    lhs[offset + 4] = static_cast<std::uint8_t>(qh >> 16);
    lhs[offset + 5] = static_cast<std::uint8_t>(qh >> 24);
    for (std::size_t index = 0; index < 16; ++index) {
      const std::uint8_t low =
          static_cast<std::uint8_t>((row + index) % 16u);
      const std::uint8_t high =
          static_cast<std::uint8_t>((row * 3u + index) % 16u);
      lhs[offset + 6 + index] = static_cast<std::uint8_t>(low | (high << 4));
    }
  }
  ...
  std::cout << "kernel-target-aarch64-q5-0-vector-live/v1\nsource_repository=stateforward/emel.cpp\nsource_commit="
  ...
  std::cout << "case=q5_0_vector_m5_k32 status=ok output_bits=";
  write_bits(output);
  std::cout << '\n';

  std::array<float, 1> zero = {k_sentinel};
  emel::kernel::event::op_mul_mat invalid{};
  invalid.src0 = view(zero.data(), emel::kernel::event::dtype::q5_0, {0, 0, 1, 1}, {1, 0, 0, 0});
  ...
  invalid.src0 = view(lhs.data(), emel::kernel::event::dtype::q5_0, {31, m, 1, 1},
                      {1, q5_block_bytes, q5_block_bytes * m, q5_block_bytes * m});
