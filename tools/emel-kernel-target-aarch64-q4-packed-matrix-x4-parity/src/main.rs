//! Split Rust observer for the public AArch64 q4 packed bl8 matrix_x4 router.
...
use emel_kernels::aarch64::{Kernel, MulMatQ4PackedBl8MatrixX4, Q4PackedBl4Error};
...
fn main() {
    eprintln!("AArch64 q4 packed bl8 matrix_x4 observer requires an AArch64 build");
    std::process::exit(2);
}
...
#[cfg(target_arch = "aarch64")]
fn main() {
    println!("kernel-target-aarch64-q4-packed-matrix-x4-live/v1");
    ...
    println!("scope=target_router_q4_packed_bl8_matrix_x4_m8_k256_and_typed_rejection");
    ...
    assert_eq!(
        actor.process_event(
            MulMatQ4PackedBl8MatrixX4::new(&lhs, &rhs, M, K),
            &mut output
        ),
        Ok(())
    );
    println!(
        "case=q4_packed_bl8_matrix_x4_m8_k256 status=ok output_bits={}",
        bits(&output)
    );
    ...
    assert_eq!(
        actor.process_event(
            MulMatQ4PackedBl8MatrixX4::new(&[], &[], 0, 0),
            &mut zero_output
        ),
    ...
    assert_eq!(
        actor.process_event(
            MulMatQ4PackedBl8MatrixX4::new(&[], &[], M, 0),
            &mut k_zero_output
        ),
        Err(Q4PackedBl4Error::InvalidShape)
    );
    ...
    assert_eq!(
        actor.process_event(
            MulMatQ4PackedBl8MatrixX4::new(&lhs, &short_rhs, M, K),
            &mut invalid_output
        ),
        Err(Q4PackedBl4Error::InvalidShape)
    );
