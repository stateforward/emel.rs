//! Split Rust observer for the public AArch64 q6 prepared q8 rhs matrix_x8 router.
...
use emel_kernels::aarch64::{Kernel, MulMatQ6PreparedMatrixX8, Q6PreparedError};
...
fn main() {
    eprintln!("AArch64 q6 prepared matrix_x8 observer requires an AArch64 build");
    std::process::exit(2);
}
...
#[cfg(target_arch = "aarch64")]
fn main() {
    println!("kernel-target-aarch64-q6-prepared-matrix-x8-live/v1");
    ...
    println!("scope=target_router_q6_prepared_matrix_x8_m8_k256_and_typed_rejection");
    println!("execution=split_pinned_aarch64_sm_and_public_target_router");
    println!("live_path=execute_neon_mul_mat_q6_vector_prepared_q8_rhs_i8mm_matrix_x8");
    ...
    assert_eq!(
        actor.process_event(MulMatQ6PreparedMatrixX8::new(&lhs, &rhs, M, K), &mut output),
        Ok(())
    );
    println!(
        "case=q6_prepared_matrix_x8_m8_k256 status=ok output_bits={}",
        bits(&output)
    );

    let mut zero_output = [SENTINEL; 8];
    assert_eq!(
        actor.process_event(
            MulMatQ6PreparedMatrixX8::new(&[], &[], 0, 0),
            &mut zero_output
        ),
    ...
    assert_eq!(
        actor.process_event(
            MulMatQ6PreparedMatrixX8::new(&[], &[], M, 0),
            &mut k_zero_output
        ),
        Err(Q6PreparedError::InvalidShape)
    );
    ...
    assert_eq!(
        actor.process_event(
            MulMatQ6PreparedMatrixX8::new(&lhs, &short_rhs, M, K),
            &mut invalid_output
        ),
        Err(Q6PreparedError::InvalidShape)
    );
