//! Split Rust observer for the public AArch64 target duplication router.

#![cfg_attr(not(target_arch = "aarch64"), allow(dead_code))]

use emel_kernels::aarch64::{Dup, DupF32Error, Kernel};
...
fn main() {
    eprintln!("AArch64 dup observer requires an AArch64 build");
    std::process::exit(2);
}
...
fn main() {
    ...
    println!("kernel-target-aarch64-dup-f32-live/v1");
    ...
    println!("scope=target_router_dup_f32_dense_body_tail_and_rejection");
    ...
    assert_eq!(actor.process_event(Dup::new(&input), &mut output), Ok(()));
    println!("case=dup status=ok output_bits={}", bits(&output));

    let mut zero_output = [];
    assert_eq!(
        actor.process_event(Dup::new(&[]), &mut zero_output),
        Err(DupF32Error::InvalidShape)
    );
    println!("case=zero_count status=reject error=InvalidShape output_bits=");

    let mut invalid_output = [SENTINEL; 2];
    assert_eq!(
        actor.process_event(Dup::new(&[1.0, 2.0, 3.0]), &mut invalid_output),
        Err(DupF32Error::InvalidShape)
    );
    assert!(
        invalid_output
