...
constexpr std::size_t q6_bytes = 210;
...
void pack_q6(::emel::kernel::detail::quant::block_q6_k &block, std::uint16_t scale,
             std::uint8_t ql, std::uint8_t qh, std::int8_t group_scale) {
  block.ql.fill(ql);
  block.qh.fill(qh);
  block.scales.fill(group_scale);
  block.d = scale;
}
...
int main() {
  std::array<::emel::kernel::detail::quant::block_q6_k, m> lhs_blocks{};
  pack_q6(lhs_blocks[0], 0x3c00, 0x12, 0x34, 2);
  pack_q6(lhs_blocks[1], 0x4000, 0x56, 0x78, -1);
  ...
  constexpr std::array<std::uint64_t, 4> lhs_nb = {1, q6_bytes, q6_bytes * m, q6_bytes * m};
  ...
  std::cout << "kernel-target-aarch64-q6-mul-mat-live/v1\nsource_repository=stateforward/emel.cpp\nsource_commit="
  ...
  std::cout << "case=q6_mul_mat_m2_k256_n2 status=ok output_bits=";
  write_bits(output);
  std::cout << '\n';
  ...
  invalid.src0 = view(lhs_blocks.data(), emel::kernel::event::dtype::q6_k, {0, m, 1, 1},
                      {1, q6_bytes, q6_bytes * m, q6_bytes * m});
