...
constexpr std::size_t q3_bytes = 110;
...
void pack_q3(::emel::kernel::detail::quant::block_q3_k &block, std::uint16_t d,
             std::uint8_t hmask, std::uint8_t qs, std::uint8_t scales) {
  block.hmask.fill(hmask);
  block.qs.fill(qs);
  block.scales.fill(scales);
  block.d = d;
}
...
int main() {
  std::array<::emel::kernel::detail::quant::block_q3_k, m> lhs_blocks{};
  pack_q3(lhs_blocks[0], 0x3c00, 0xa5, 0xe4, 0x12);
  pack_q3(lhs_blocks[1], 0x4000, 0x5a, 0x1b, 0x34);
  ...
  constexpr std::array<std::uint64_t, 4> lhs_nb = {1, q3_bytes, q3_bytes * m, q3_bytes * m};
  ...
  std::cout << "kernel-target-aarch64-q3-k-mul-mat-live/v1\nsource_repository=stateforward/emel.cpp\nsource_commit="
  ...
  std::cout << "case=q3_k_mul_mat_m2_k256_n2 status=ok output_bits=";
  write_bits(output);
  std::cout << '\n';

  std::array<float, 4> zero = {k_sentinel, k_sentinel, k_sentinel, k_sentinel};
  emel::kernel::event::op_mul_mat invalid{};
  invalid.src0 = view(zero.data(), emel::kernel::event::dtype::q3_k, {0, 0, 1, 1}, {1, 0, 0, 0});
  ...
  invalid.src0 = view(lhs_blocks.data(), emel::kernel::event::dtype::q3_k, {0, m, 1, 1},
                      {1, q3_bytes, q3_bytes * m, q3_bytes * m});
