...
/// Numeric processor error retained by the bind phase.
///
/// Callback-provided values are preserved rather than narrowed to the built-in
/// processor error set.
...
impl ProcessorError {
    ...
    #[allow(non_upper_case_globals)]
    pub const InvalidRequest: Self = Self::INVALID_REQUEST;
    #[allow(non_upper_case_globals)]
    pub const KernelFailed: Self = Self::KERNEL_FAILED;
    #[allow(non_upper_case_globals)]
    pub const InternalError: Self = Self::INTERNAL_ERROR;
    #[allow(non_upper_case_globals)]
    pub const Untracked: Self = Self::UNTRACKED;
}
...
pub struct ExecuteRequest {
    ...
    pub root_bind_inputs: Option<crate::processor::sm::BindInputsFn>,
}
...
impl GraphProcessorBindStepStateMachineContext for GraphProcessorBindStepContext {
    ...
    fn run_callback(&mut self) -> Result<(), ()> {
        let callback = self.request.bind_inputs.ok_or(())?;
        self.phase_callback_err = 0;
        self.phase_callback_ok = callback(&self.request, &mut self.phase_callback_err);
        Ok(())
    }
}
...
impl GraphProcessorBindStepActor {
    ...
    pub fn new() -> Self {
        Self {
            machine: GraphProcessorBindStepStateMachine::new(
                GraphProcessorBindStepContext::default(),
            ),
        }
    }
    ...
    pub fn process_event(&mut self, event: ProcessorEventExecuteStep) -> bool {
        ...
        let origin = GraphProcessorBindStepCompletionOrigin::ProcessorEventExecuteStep;
        if self.machine.process_completion(&origin) != Ok(true) {
            return false;
        }
        if self
            .machine
            .is(&GraphProcessorBindStepStates::CallbackDecision)
        {
    ...
    #[must_use]
    pub fn state(&self) -> &GraphProcessorBindStepStates {
        self.machine.state()
    }

    /// Reports whether the generated machine is in `state`.
    #[must_use]
    pub fn is(&self, state: &GraphProcessorBindStepStates) -> bool {
        self.machine.is(state)
    }

    /// Returns the retained bounded context.
    #[must_use]
    pub fn context(&self) -> &GraphProcessorBindStepContext {
...
mod tests {
    ...
    fn callback_success_is_retained() {
        let mut actor = GraphProcessorBindStepActor::new();
        assert!(actor.process_event(ProcessorEventExecuteStep {
            request: ExecuteRequest {
                bind_inputs: Some(succeeds),
    ...
    fn callback_failure_modes_are_distinct() {
        ...
        assert!(actor.process_event(ProcessorEventExecuteStep {
            request: ExecuteRequest {
                bind_inputs: Some(fails_with_error),
                ..Default::default()
            },
            initial_error: ProcessorError::None,
        }));
        assert_eq!(actor.context().bind_outcome, PhaseOutcome::Failed);
        assert_eq!(actor.context().err, ProcessorError::from_callback_error(4));

        let mut actor = GraphProcessorBindStepActor::new();
        assert!(actor.process_event(ProcessorEventExecuteStep {
            request: ExecuteRequest {
                bind_inputs: Some(fails_without_error),
                ..Default::default()
            },
            initial_error: ProcessorError::None,
        }));
