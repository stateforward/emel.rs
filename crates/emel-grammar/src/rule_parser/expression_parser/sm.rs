...
impl Default for GbnfRuleParserExpressionParserContext {
    fn default() -> Self {
        Self {
            result: ParseOutcome::Parsed(ParseKind::Unknown),
        }
    }
}
...
impl GbnfRuleParserExpressionParserStateMachineContext for GbnfRuleParserExpressionParserContext {
    ...
    fn parse_failed(&self, event: &RuleParserEventParseRules) -> Result<bool, ()> {
        Ok(!is_supported(event.token_kind))
    }
    fn token_identifier(&self, event: &RuleParserEventParseRules) -> Result<bool, ()> {
        Ok(event.token_kind == Some(TokenKind::Identifier))
    }
...
impl GbnfRuleParserExpressionParserActor {
    ...
    pub fn new() -> Self {
        Self {
            machine: GbnfRuleParserExpressionParserStateMachine::new(
                GbnfRuleParserExpressionParserContext::default(),
            ),
        }
    }
    ...
    pub fn process_event(&mut self, event: RuleParserEventParseRules) -> ParseOutcome {
        if !self
            .machine
            .is(&GbnfRuleParserExpressionParserStates::Deciding)
        {
        ...
        let failed = self
            .machine
            .process_event(GbnfRuleParserExpressionParserEvents::RuleParserEventParseRules(event))
            .is_err()
    ...
    #[must_use]
    pub fn state(&self) -> &GbnfRuleParserExpressionParserStates {
        self.machine.state()
    }

    /// Reports whether the generated machine is in `state`.
    #[must_use]
    pub fn is(&self, state: &GbnfRuleParserExpressionParserStates) -> bool {
...
mod tests {
    ...
    fn classifies_identifier() {
        let mut parser = GbnfRuleParserExpressionParserActor::new();
        assert_eq!(
            parser.classify(TokenKind::Identifier),
            ParseOutcome::Parsed(ParseKind::Identifier)
        );
    ...
    fn classifies_non_identifier() {
        let mut parser = GbnfRuleParserExpressionParserActor::new();
        assert_eq!(
            parser.classify(TokenKind::StringLiteral),
            ParseOutcome::Parsed(ParseKind::NonIdentifier)
        );
        assert!(parser.is(&GbnfRuleParserExpressionParserStates::X));
    }
    ...
    fn rejects_unsupported_and_absent_input() {
        ...
        let mut parser = GbnfRuleParserExpressionParserActor::new();
        assert_eq!(
            parser.process_event(RuleParserEventParseRules::absent()),
            ParseOutcome::ParseFailed
        );
        assert!(parser.is(&GbnfRuleParserExpressionParserStates::X));
    }
