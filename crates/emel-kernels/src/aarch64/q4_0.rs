//! Safe `AArch64` NEON `q4_0` vector matmul for the pinned kernel contract.
//!
//! The source-backed boundary is the single-RHS route selected by
//! `can_run_neon_mul_mat_q4_0_vector_request`: packed `q4_0` `src0` with shape
...
use crate::any::quant::{BLOCK_VALUES, Q4_0_BLOCK_BYTES, fp16_to_f32, fp32_to_fp16};

/// Errors returned by the `AArch64` `q4_0` vector actor.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum Q4_0VectorError {
...
impl fmt::Display for Q4_0VectorError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::BackendUnavailable => formatter.write_str("q4_0 vector NEON backend unavailable"),
            Self::InvalidShape => formatter.write_str("invalid q4_0 vector shape"),
            Self::UnexpectedEvent => formatter.write_str("unexpected q4_0 vector event"),
            Self::Internal => formatter.write_str("internal q4_0 vector dispatch error"),
        }
    }
}

impl std::error::Error for Q4_0VectorError {}

/// A pinned `AArch64` packed `q4_0` by dense F32 vector request.
///
/// `lhs` is row-major packed `q4_0` with `m` rows of `k / 32` blocks.
/// `rhs` is one dense F32 vector of length `k`.
#[derive(Debug)]
pub struct OpMulMatQ4_0Vector<'a> {
    lhs: &'a [u8],
    rhs: &'a [f32],
    m: usize,
    k: usize,
...
/// Explicitly reports an event outside the `q4_0` vector API.
#[derive(Clone, Copy, Debug, Default)]
pub struct UnexpectedQ4_0Vector;

/// Result returned by target `q4_0` vector events.
pub type Q4_0VectorResult = Result<(), Q4_0VectorError>;
...
const PINNED_GUARD_SPAN: &str = "src/emel/kernel/aarch64/guards.hpp:353-362";
const PINNED_ACTION_SPAN: &str = "src/emel/kernel/aarch64/actions.hpp:1013-1063";
const PINNED_EXECUTION_SPAN: &str = "src/emel/kernel/aarch64/actions.hpp:7184-7243";
const PINNED_DOT_SPAN: &str = "src/emel/kernel/aarch64/actions.hpp:3951-4091";
const PINNED_TRANSITION_SPAN: &str = "src/emel/kernel/aarch64/sm.hpp:360-363";
const SCOPE_RESIDUAL: &str = "q4_0 vector only (packed src0[k,m], dense F32 src1[1,k], dense F32 dst[1,m]); q4_1/q5/q6/q8 SIMD, F16 neon, and x86 remain residuals";

const MAX_Q8_0_BLOCKS: usize = 1024;

/// Event implemented by the target `q4_0` vector actor.
pub trait Q4_0VectorEvent: Sized {
    /// Result returned after RTC dispatch.
    type Output;

    #[doc(hidden)]
    fn dispatch(self, actor: &mut Q4_0VectorKernel, output: &mut [f32]) -> Self::Output;
}
...
struct Q4_0VectorRuntime<'a> {
    event: OpMulMatQ4_0Vector<'a>,
    output: &'a mut [f32],
    result: &'a Cell<Q4_0VectorResult>,
}
...
sml! {
    Q4_0VectorMachine<'dispatch> {
        "ready"_s <= *"ready"_s + Q4_0Vector(Q4_0VectorRuntime<'dispatch>) [guard_q4_0_ready] / effect_q4_0_execute,
        "ready"_s <= "ready"_s + Q4_0Vector(Q4_0VectorRuntime<'dispatch>) [guard_q4_0_invalid] / effect_q4_0_invalid,
...
/// Single-writer, run-to-completion `AArch64` `q4_0` vector actor.
pub struct Q4_0VectorKernel {
    machine: Q4_0VectorMachineStateMachine<Context>,
}

impl fmt::Debug for Q4_0VectorKernel {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("Q4_0VectorKernel")
            .finish_non_exhaustive()
    }
}

impl Q4_0VectorKernel {
    ...
    pub fn try_new() -> Option<Self> {
        ...
        Some(Self {
            machine: Q4_0VectorMachineStateMachine::new(Context {
    ...
    pub fn process_event<E: Q4_0VectorEvent>(&mut self, event: E, output: &mut [f32]) -> E::Output {
    ...
    pub fn is_ready(&self) -> bool {
        self.machine.is(&Q4_0VectorMachineStates::Ready)
    }

    fn q4_0_vector(
        &mut self,
        event: OpMulMatQ4_0Vector<'_>,
        output: &mut [f32],
    ) -> Q4_0VectorResult {
        let result = Cell::new(Err(Q4_0VectorError::UnexpectedEvent));
        self.machine
            .process_event(Q4_0VectorMachineEvents::Q4_0Vector(Q4_0VectorRuntime {
                event,
                output,
                result: &result,
            }))
...
impl Q4_0VectorEvent for OpMulMatQ4_0Vector<'_> {
    type Output = Q4_0VectorResult;

    fn dispatch(self, actor: &mut Q4_0VectorKernel, output: &mut [f32]) -> Self::Output {
        actor.q4_0_vector(self, output)
    }
}

impl Q4_0VectorEvent for UnexpectedQ4_0Vector {
    type Output = Q4_0VectorResult;

    fn dispatch(self, actor: &mut Q4_0VectorKernel, _output: &mut [f32]) -> Self::Output {
        let result = Cell::new(Err(Q4_0VectorError::Internal));
        actor
            .machine
            .process_event(Q4_0VectorMachineEvents::Unexpected(UnexpectedRuntime {
                result: &result,
            }))
            .map_err(|_| Q4_0VectorError::Internal)?;
        result.get()
...
impl Q4_0VectorMachineStateMachineContext for Context {
    fn guard_q4_0_ready(&self, event: &Q4_0VectorRuntime<'_>) -> Result<bool, ()> {
        Ok(self.backend_available && q4_0_request_valid(event))
    }

    fn guard_q4_0_invalid(&self, event: &Q4_0VectorRuntime<'_>) -> Result<bool, ()> {
        Ok(!q4_0_request_valid(event))
    }

    fn effect_q4_0_execute(&mut self, event: Q4_0VectorRuntime<'_>) -> Result<(), ()> {
        execute_q4_0_vector(
            self.backend,
            event.event.lhs,
            event.event.rhs,
    ...
    fn effect_q4_0_invalid(&mut self, event: Q4_0VectorRuntime<'_>) -> Result<(), ()> {
        event.result.set(Err(Q4_0VectorError::InvalidShape));
        Ok(())
    }

    fn effect_unexpected(&mut self, event: UnexpectedRuntime<'_>) -> Result<(), ()> {
        event.result.set(Err(Q4_0VectorError::UnexpectedEvent));
        Ok(())
    }
...
const fn q4_0_request_valid(event: &Q4_0VectorRuntime<'_>) -> bool {
    ...
    let Some(row_bytes) = blocks.checked_mul(Q4_0_BLOCK_BYTES) else {
        return false;
    };
...
fn execute_q4_0_vector(
    ...
    while row < m {
        ...
        output[row] = dot_q4_0_q8_0_neon(backend, packed, &scratch[..blocks]);
        row += 1;
    }
}
...
fn dot_q4_0_q8_0_neon(backend: Neon, row: &[u8], rhs: &[Q8Scratch]) -> f32 {
    let mut sum = 0.0_f32;
    let mut block = 0;
    while block < rhs.len() {
        let offset = block * Q4_0_BLOCK_BYTES;
        let mut packed = [0_u8; 16];
        packed.copy_from_slice(&row[offset + 2..offset + Q4_0_BLOCK_BYTES]);
        let qs: u8x16 = cast(packed);
        let low_mask = backend.splat_u8x16(0x0f);
        let bias = backend.splat_i8x16(8);
        let lhs_lo = backend.wrapping_sub_i8x16(cast(backend.and_u8x16(qs, low_mask)), bias);
        let lhs_hi = backend.wrapping_sub_i8x16(cast(backend.shr_const_u8x16::<4>(qs)), bias);
...
mod tests {
    ...
    use super::{
        BLOCK_VALUES, MAX_Q8_0_BLOCKS, OpMulMatQ4_0Vector, Q4_0_BLOCK_BYTES, Q4_0VectorError,
        Q4_0VectorEvent, Q4_0VectorKernel, UnexpectedQ4_0Vector, quantize_q8_0_dense,
    };
    use crate::any::quant::{Q4_0Row, Q8_0_BLOCK_BYTES, Q8_0Row, dot_q4_0_q8_0, fp32_to_fp16};
    use allocation_counter::measure;

    fn pack_q4_row(scale: u16, lows: [u8; 16], highs: [u8; 16]) -> [u8; Q4_0_BLOCK_BYTES] {
        let mut block = [0_u8; Q4_0_BLOCK_BYTES];
        block[..2].copy_from_slice(&scale.to_le_bytes());
        let mut index = 0;
        while index < 16 {
            block[2 + index] = (lows[index] & 0x0f) | ((highs[index] & 0x0f) << 4);
            index += 1;
        }
        block
    }
    ...
    fn pack_q8_from_scratch(d: u16, qs: [i8; BLOCK_VALUES]) -> [u8; Q8_0_BLOCK_BYTES] {
        ...
        let mut index = 0;
        while index < BLOCK_VALUES {
            block[2 + index] = qs[index].to_ne_bytes()[0];
            index += 1;
        }
        block
    }
    ...
    fn target_capability_is_resolved_before_dispatch() {
        assert!(Q4_0VectorKernel::try_new().is_some());
    }
    ...
    fn pinned_source_identity_is_explicit() {
        ...
        assert_eq!(
            ...
            "src/emel/kernel/aarch64/guards.hpp:353-362"
        );
        assert_eq!(
            super::PINNED_ACTION_SPAN,
            "src/emel/kernel/aarch64/actions.hpp:1013-1063"
        );
        assert_eq!(
            super::PINNED_EXECUTION_SPAN,
            "src/emel/kernel/aarch64/actions.hpp:7184-7243"
        );
        assert_eq!(
            super::PINNED_DOT_SPAN,
            "src/emel/kernel/aarch64/actions.hpp:3951-4091"
        );
        assert_eq!(
            super::PINNED_TRANSITION_SPAN,
            "src/emel/kernel/aarch64/sm.hpp:360-363"
        );
        assert!(super::SCOPE_RESIDUAL.contains("q4_0 vector only"));
        assert_eq!(MAX_Q8_0_BLOCKS, 1024);
    }

    #[test]
    fn neon_integer_dot_matches_packed_scalar_oracle() {
        let Some(mut kernel) = Q4_0VectorKernel::try_new() else {
            return;
        };
        ...
        let lhs = pack_q4_row(0x3c00, lows, highs);
        ...
        assert_eq!(
            kernel.process_event(
                OpMulMatQ4_0Vector::new(&lhs, &rhs, 1, BLOCK_VALUES),
                &mut output
            ),
            Ok(())
        );
    ...
    fn four_plus_remainder_rows_match_scalar_oracle() {
        ...
        let mut lhs = [0_u8; 5 * Q4_0_BLOCK_BYTES];
        let mut row = 0;
        while row < 5 {
            let mut lows = [0_u8; 16];
            let mut highs = [0_u8; 16];
            ...
            let packed = pack_q4_row(fp32_to_fp16(1.0 + row as f32 * 0.25), lows, highs);
            lhs[row * Q4_0_BLOCK_BYTES..(row + 1) * Q4_0_BLOCK_BYTES].copy_from_slice(&packed);
            row += 1;
        }
        let mut rhs = [0.0_f32; BLOCK_VALUES];
        let mut index = 0;
        ...
        assert_eq!(
            kernel.process_event(
                OpMulMatQ4_0Vector::new(&lhs, &rhs, 5, BLOCK_VALUES),
                &mut output
            ),
            Ok(())
        );
        ...
        row = 0;
        while row < 5 {
            let packed = &lhs[row * Q4_0_BLOCK_BYTES..(row + 1) * Q4_0_BLOCK_BYTES];
            let expected = dot_q4_0_q8_0(
                Q4_0Row::from_bytes(packed).unwrap(),
                Q8_0Row::from_bytes(&q8).unwrap(),
            )
            .unwrap();
    ...
    fn invalid_shape_does_not_mutate_output() {
        ...
        assert_eq!(
            kernel.process_event(OpMulMatQ4_0Vector::new(&[1, 2], &[3.0], 2, 2), &mut output),
            Err(Q4_0VectorError::InvalidShape)
        );
    ...
    fn unexpected_event_is_explicit() {
        ...
        assert_eq!(
            kernel.process_event(UnexpectedQ4_0Vector, &mut []),
            Err(Q4_0VectorError::UnexpectedEvent)
        );
    }
    ...
    fn event_trait_remains_dispatch_surface() {
        ...
        let lhs = pack_q4_row(0x3c00, [8; 16], [8; 16]);
        let rhs = [1.0_f32; BLOCK_VALUES];
        let mut output = [7.0];
        assert_eq!(
            Q4_0VectorEvent::dispatch(
                OpMulMatQ4_0Vector::new(&lhs, &rhs, 1, BLOCK_VALUES),
                &mut kernel,
                &mut output
            ),
