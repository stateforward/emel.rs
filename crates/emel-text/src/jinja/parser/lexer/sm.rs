...
fn scan<'a, 'cb>(event: EventNextRuntime<'a, 'cb>) {
    ...
    if b[pos]==b'\'' || b[pos]==b'"' {
        ...
        while pos<b.len() && b[pos]!=term { if b[pos]==b'\\' { pos+=1; if pos>=b.len(){emit_error(event.dispatch_error,Error::ParseFailed,pos);return;} let ch=match b[pos]{b'n'=>'\n',b't'=>'\t',b'r'=>'\r',b'b'=>'\x08',b'f'=>'\x0c',b'v'=>'\x0b',b'\\'=>'\\',b'\''=>'\'',b'"'=>'"',_=>{emit_error(event.dispatch_error,Error::ParseFailed,pos);return;}}; value.push(ch); pos+=1; } else { let width=if b[pos]<0x80{1}else if b[pos]<0xE0{2}else if b[pos]<0xF0{3}else{4}; if pos+width>b.len(){emit_error(event.dispatch_error,Error::ParseFailed,pos);return;} let _=core::str::from_utf8(&b[pos..pos+width]).map_or_else(|_|{emit_error(event.dispatch_error,Error::ParseFailed,pos);false},|v|{value.push_str(v);true}); pos+=width; } }
        if pos>=b.len(){emit_error(event.dispatch_error,Error::ParseFailed,pos);return;} pos+=1; emit_token(event.dispatch_done,c,Token{kind:TokenKind::StringLiteral,value:value,pos:start},pos,false,false); return;
    }
...
fn emit_token_with_flags<'a,'cb>(cb:Option<&'cb mut DoneCallback<'a>>,c:Cursor<'a>,tok:Token,end:usize,r:bool,t:bool){let mut n=c;n.offset=end;n.token_index=c.token_index.saturating_add(1);n.last_token_type=tok.kind;n.last_block_can_trim_newline=t||tok.kind==TokenKind::Comment;n.last_block_rstrip=r;if tok.kind==TokenKind::OpenExpression{n.curly_bracket_depth=0;}else if tok.kind==TokenKind::OpenCurlyBracket{n.curly_bracket_depth=c.curly_bracket_depth.saturating_add(1);}else if tok.kind==TokenKind::CloseCurlyBracket{n.curly_bracket_depth=c.curly_bracket_depth.saturating_sub(1);}emit_done(cb,NextDone{token:tok,has_token:true,next_cursor:n});}
...
fn mapping<'a>(s:&'a str,p:usize,depth:usize)->Option<(TokenKind,usize,bool,bool)>{let r=&s[p..];let a:[(&str,TokenKind,bool,bool);31]=[("{%-",TokenKind::OpenStatement,false,false),("-%}",TokenKind::CloseStatement,true,true),("{{-",TokenKind::OpenExpression,false,false),("-}}",TokenKind::CloseExpression,true,true),("{%",TokenKind::OpenStatement,false,false),("%}",TokenKind::CloseStatement,false,true),("{{",TokenKind::OpenExpression,false,false),("}}",TokenKind::CloseExpression,false,true),("<=",TokenKind::ComparisonBinaryOperator,false,false),(">=",TokenKind::ComparisonBinaryOperator,false,false),("==",TokenKind::ComparisonBinaryOperator,false,false),("!=",TokenKind::ComparisonBinaryOperator,false,false),("(",TokenKind::OpenParen,false,false),(")",TokenKind::CloseParen,false,false),("[",TokenKind::OpenSquareBracket,false,false),("]",TokenKind::CloseSquareBracket,false,false),("{",TokenKind::OpenCurlyBracket,false,false),("}",TokenKind::CloseCurlyBracket,false,false),(",",TokenKind::Comma,false,false),(".",TokenKind::Dot,false,false),(":",TokenKind::Colon,false,false),("|",TokenKind::Pipe,false,false),("<",TokenKind::ComparisonBinaryOperator,false,false),(">",TokenKind::ComparisonBinaryOperator,false,false),("+",TokenKind::AdditiveBinaryOperator,false,false),("-",TokenKind::AdditiveBinaryOperator,false,false),("~",TokenKind::AdditiveBinaryOperator,false,false),("*",TokenKind::MultiplicativeBinaryOperator,false,false),("/",TokenKind::MultiplicativeBinaryOperator,false,false),("%",TokenKind::MultiplicativeBinaryOperator,false,false),("=",TokenKind::Equals,false,false)];for&(x,k,rstrip,trim)in&a{if r.starts_with(x){if k==TokenKind::CloseExpression&&depth>0{continue;}return Some((k,x.len(),rstrip,trim));}}None}
