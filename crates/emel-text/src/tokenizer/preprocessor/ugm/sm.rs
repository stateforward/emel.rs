//! Source-aligned, bounded UGM tokenizer preprocessor actor.
//!
//! The machine follows the pinned `preprocessor::ugm` transition table.  All
...
sml! {
    TextTokenizerPreprocessorUgm<'event> {
...
impl<'event> TextTokenizerPreprocessorUgmStateMachineContext
    for TextTokenizerPreprocessorUgmContext
{
    ...
    fn unexpected(&mut self) -> Result<(), ()> {
        ...
        self.error = PreprocessorError::InvalidRequest;
        self.result = false;
        Ok(())
    }
}
...
pub type TextTokenizerPreprocessorUgmContext = PreprocessContext;
...
/// Synchronous bounded actor around the generated UGM machine.
pub struct TextTokenizerPreprocessorUgmActor<'event> {
    machine: TextTokenizerPreprocessorUgmStateMachine<'event, TextTokenizerPreprocessorUgmContext>,
}

impl<'event> Default for TextTokenizerPreprocessorUgmActor<'event> {
    fn default() -> Self { Self::new() }
}

impl<'event> TextTokenizerPreprocessorUgmActor<'event> {
    /// Creates an actor in the generated `idle` state.
    #[must_use]
    pub fn new() -> Self {
        Self { machine: TextTokenizerPreprocessorUgmStateMachine::new(TextTokenizerPreprocessorUgmContext::default()) }
    }
    ...
    pub fn process_event(&mut self, event: EventPreprocessRuntime<'event>) -> bool {
        ...
        let accepted = self.machine.process_event(TextTokenizerPreprocessorUgmEvents::EventPreprocessRuntime(event)).is_ok();
        let ok = accepted && context.borrow().result && self.machine.is(&TextTokenizerPreprocessorUgmStates::Done);
    ...
    pub fn process_unexpected(&mut self) -> bool {
        let _ = self.machine.context_mut().unexpected = true;
        self.machine.set_state(TextTokenizerPreprocessorUgmStates::Unexpected);
        false
    }

    /// Returns the generated state.
    #[must_use]
    pub fn state(&self) -> &TextTokenizerPreprocessorUgmStates { self.machine.state() }
    /// Reports whether the actor is in `state`.
    #[must_use]
    pub fn is(&self, state: &TextTokenizerPreprocessorUgmStates) -> bool { self.machine.is(state) }
    /// Returns the machine context.
    #[must_use]
    pub fn context(&self) -> &TextTokenizerPreprocessorUgmContext { self.machine.context() }
}
