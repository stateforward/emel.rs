...
sml! {
    TextTokenizerPreprocessorUgm<'dispatch, 'event>
    where
        'event: 'dispatch, {
        ...
        'event: 'dispatch,
    {
        ...
        while i < count {
            ...
            while j > 0 {
                ...
                let previous_len = event.request.vocab.entries[previous].text.len();
                if previous_len >= len {
                    break;
                }
...
pub struct TextTokenizerPreprocessorUgmActor {
    machine: TextTokenizerPreprocessorUgmStateMachine<TextTokenizerPreprocessorUgmContext>,
    last_error: PreprocessorError,
    fragment_count: usize,
}

impl Default for TextTokenizerPreprocessorUgmActor {
    fn default() -> Self {
        Self::new()
    }
}

impl TextTokenizerPreprocessorUgmActor {
    /// Creates an actor in the generated `idle` state.
    #[must_use]
    pub fn new() -> Self {
        Self {
            machine: TextTokenizerPreprocessorUgmStateMachine::new(
                TextTokenizerPreprocessorUgmContext::default(),
            ),
        ...
        'event: 'dispatch,
    {
        ...
        let accepted = self
            .machine
            .process_event(TextTokenizerPreprocessorUgmEvents::EventPreprocessRuntime(
                &event,
            ))
            .is_ok();
        let ok = accepted
            && context.borrow().result
            && self.machine.is(&TextTokenizerPreprocessorUgmStates::Done);
    ...
    pub fn process_unexpected(&mut self) -> bool {
        ...
        context.fragment_count = 0;
        self.machine
            .set_state(TextTokenizerPreprocessorUgmStates::Unexpected);
        false
    }

    /// Returns the last source-compatible error.
    #[must_use]
    pub const fn last_error(&self) -> PreprocessorError {
        self.last_error
    }

    /// Returns the last produced fragment count.
    #[must_use]
    ...
    #[must_use]
    pub fn state(&self) -> &TextTokenizerPreprocessorUgmStates {
        self.machine.state()
    }
    /// Reports whether the actor is in `state`.
    #[must_use]
    pub fn is(&self, state: &TextTokenizerPreprocessorUgmStates) -> bool {
        self.machine.is(state)
    }
    /// Returns the machine context.
    #[must_use]
    pub fn context(&self) -> &TextTokenizerPreprocessorUgmContext {
