...
impl<'a> EventEncodeRuntime<'a> {
    ...
    pub fn new(request: &EncodeRequest<'a>) -> Self {
        Self { vocab: request.vocab, text: request.text, token_capacity: request.token_ids.len() }
    }
}
sml! {
...
impl Default for TextEncodersSpmContext {
    fn default() -> Self {
        Self { err: ERROR_OK, token_count: 0, tables_ready: false, vocab_key: 0, symbols: [Symbol::default(); MAX_ENCODE_SYMBOLS], output: [0; MAX_ENCODE_TOKENS], transformed: [0; MAX_ENCODE_BYTES], transformed_len: 0 }
    }
}

impl TextEncodersSpmContext {
    fn reset(&mut self) { self.err = ERROR_OK; self.token_count = 0; self.transformed_len = 0; self.output.fill(0); self.symbols.fill(Symbol::default()); }
    ...
    fn push(&mut self, token_capacity: usize, token: i32) -> bool { let index = usize::try_from(self.token_count).unwrap_or(MAX_ENCODE_TOKENS); if index >= MAX_ENCODE_TOKENS || index >= token_capacity { return false; } self.output[index] = token; self.token_count += 1; true }
    fn prepare(&mut self, event: &EventEncodeRuntime<'_>) {
        let text = event.text;
        self.transformed_len = 0;
        if text.len() > MAX_ENCODE_BYTES { self.err = ERROR_INVALID_ARGUMENT; return; }
        ...
        for byte in text.bytes() {
            if prefix && !prefixed && byte != b' ' {
                if !Self::append_space(&mut self.transformed, &mut out, vocab.escape_whitespaces) { self.err = ERROR_INVALID_ARGUMENT; return; }
                prefixed = true;
            }
            if byte == b' ' {
                if !Self::append_space(&mut self.transformed, &mut out, vocab.escape_whitespaces) { self.err = ERROR_INVALID_ARGUMENT; return; }
        ...
        if suffix && !Self::append_space(&mut self.transformed, &mut out, vocab.escape_whitespaces) { self.err = ERROR_INVALID_ARGUMENT; return; }
        self.transformed_len = out;
        let mut offset = 0usize;
        while offset < out {
            let Some(index) = self.symbols.iter().position(|symbol| !symbol.alive) else { self.err = ERROR_INVALID_ARGUMENT; return; };
            let len = utf8_len(self.transformed[offset]).min(out - offset);
            self.symbols[index] = Symbol { start: offset, len, alive: true };
            offset += len;
        }
    }
    ...
    fn merge(&mut self, event: &EventEncodeRuntime<'_>) {
        ...
        for _ in 0..count.saturating_sub(1) {
            ...
            for left in 0..MAX_ENCODE_SYMBOLS {
                if !self.symbols[left].alive { continue; }
                let Some(right) = (left + 1..MAX_ENCODE_SYMBOLS).find(|index| self.symbols[*index].alive) else { continue; };
                let a = symbol_bytes(&self.symbols[left], text);
                let b = symbol_bytes(&self.symbols[right], text);
                let mut joined = [0u8; MAX_ENCODE_BYTES];
                if a.len() + b.len() > joined.len() { continue; }
                joined[..a.len()].copy_from_slice(a);
                joined[a.len()..a.len() + b.len()].copy_from_slice(b);
                let Ok(candidate) = core::str::from_utf8(&joined[..a.len() + b.len()]) else { continue; };
                if let Some(id) = Self::lookup(event.vocab, candidate) {
                    let score = event.vocab.scores.get(id as usize).copied().unwrap_or(f32::NEG_INFINITY);
                    if best.is_none_or(|(_, old)| score > old) { best = Some((left, score)); }
                }
            }
            let Some((left, _)) = best else { break; };
            let Some(right) = (left + 1..MAX_ENCODE_SYMBOLS).find(|index| self.symbols[*index].alive) else { break; };
    ...
    fn encode(&mut self, event: &EventEncodeRuntime<'_>) {
        for index in 0..MAX_ENCODE_SYMBOLS {
            if !self.symbols[index].alive { continue; }
            let start = self.symbols[index].start;
            let end = start.saturating_add(self.symbols[index].len).min(self.transformed_len);
            let direct = core::str::from_utf8(&self.transformed[start..end]).ok().and_then(|piece| Self::lookup(event.vocab, piece));
            if let Some(id) = direct {
                if !self.push(event.token_capacity, id) { self.err = ERROR_INVALID_ARGUMENT; return; }
            } else {
                for offset in start..end {
                    ...
                    let hex_id = core::str::from_utf8(&repr).ok().and_then(|piece| Self::lookup(event.vocab, piece));
                    let id = hex_id.or_else(|| { let single = [byte]; core::str::from_utf8(&single).ok().and_then(|piece| Self::lookup(event.vocab, piece)) });
                    let Some(id) = id else { self.err = ERROR_BACKEND; return; };
                    if !self.push(event.token_capacity, id) { self.err = ERROR_INVALID_ARGUMENT; return; }
                }
            }
        }
    ...
    fn append_result(&self, request: &mut EncodeRequest<'_>) {
        ...
        let count = if success { usize::try_from(self.token_count).unwrap_or(0).min(request.token_ids.len()) } else { 0 };
        request.token_ids[..count].copy_from_slice(&self.output[..count]);
        if let Some(out) = request.token_count_out.as_deref_mut() { *out = if success { self.token_count } else { 0 }; }
        if let Some(out) = request.error_out.as_deref_mut() { *out = self.err; }
    }
}
...
impl TextEncodersSpmStateMachineContext for TextEncodersSpmContext {
    fn apply_emit_result_failed(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.token_count = 0; Ok(()) }
    fn apply_emit_result_ok(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.err = ERROR_OK; Ok(()) }
    fn begin_encode(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.reset(); Ok(()) }
    fn begin_encode_sync_vocab(&mut self, event: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.reset(); self.vocab_key = event.vocab as *const _ as usize; self.tables_ready = false; Ok(()) }
    fn emit_result_failed(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err != ERROR_OK) }
    fn emit_result_ok(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_OK) }
    fn encode_result_backend_error(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_BACKEND) }
    fn encode_result_invalid_argument_error(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_INVALID_ARGUMENT) }
    fn encode_result_model_invalid_error(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_MODEL_INVALID) }
    fn encode_result_ok(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_OK) }
    fn encode_result_unclassified_error_code(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(is_other(self.err)) }
    fn ensure_last_error_from_emit_result_decision(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { if self.err == ERROR_OK { self.err = ERROR_BACKEND; } Ok(()) }
    fn ensure_last_error_from_encode_emit_input_decision(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { if self.err == ERROR_OK { self.err = ERROR_BACKEND; } Ok(()) }
    fn ensure_last_error_from_encode_merge_result_decision(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { if self.err == ERROR_OK { self.err = ERROR_BACKEND; } Ok(()) }
    fn ensure_last_error_from_encode_precheck_decision(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { if self.err == ERROR_OK { self.err = ERROR_BACKEND; } Ok(()) }
    fn ensure_last_error_from_encode_prepare_result_decision(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { if self.err == ERROR_OK { self.err = ERROR_BACKEND; } Ok(()) }
    fn ensure_last_error_from_encode_result_decision(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { if self.err == ERROR_OK { self.err = ERROR_BACKEND; } Ok(()) }
    fn ensure_last_error_from_table_policy_decision(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { if self.err == ERROR_OK { self.err = ERROR_BACKEND; } Ok(()) }
    fn ensure_last_error_from_table_sync_result_decision(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { if self.err == ERROR_OK { self.err = ERROR_BACKEND; } Ok(()) }
    fn invalid_encode(&self, event: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(!self.valid_encode(event)?) }
    fn mark_done_from_encode_precheck_decision(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.err = ERROR_OK; Ok(()) }
    fn mark_done_from_encode_result_decision(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.err = ERROR_OK; Ok(()) }
    fn merge_result_backend_error(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_BACKEND) }
    fn merge_result_invalid_argument_error(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_INVALID_ARGUMENT) }
    fn merge_result_model_invalid_error(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_MODEL_INVALID) }
    fn merge_result_ok(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_OK) }
    fn merge_result_unclassified_error_code(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(is_other(self.err)) }
    fn merge_symbol_capacity_exceeded(&self, event: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(event.text.len() > MAX_ENCODE_SYMBOLS) }
    fn merge_symbol_capacity_within_limit(&self, event: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(!self.merge_symbol_capacity_exceeded(event)?) }
    fn on_unexpected_events_encoding_done<'dispatch, 'event>(&mut self, _: &'dispatch EventsEncodingDone<'event>) -> Result<(), ()> where 'event: 'dispatch { self.err = ERROR_UNEXPECTED; Ok(()) }
    fn on_unexpected_events_encoding_error<'dispatch, 'event>(&mut self, _: &'dispatch EventsEncodingError<'event>) -> Result<(), ()> where 'event: 'dispatch { self.err = ERROR_UNEXPECTED; Ok(()) }
    fn on_unexpected_runtime_encode_runtime(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.err = ERROR_UNEXPECTED; Ok(()) }
    fn on_unexpected_unexp_wild(&mut self) -> Result<(), ()> { self.err = ERROR_UNEXPECTED; Ok(()) }
    fn prepare_result_backend_error(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_BACKEND) }
    fn prepare_result_invalid_argument_error(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_INVALID_ARGUMENT) }
    fn prepare_result_model_invalid_error(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_MODEL_INVALID) }
    fn prepare_result_ok(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_OK) }
    fn prepare_result_unclassified_error_code(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(is_other(self.err)) }
    fn reject_invalid_encode_from_encode_merge_input_capacity_decision(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.err = ERROR_INVALID_ARGUMENT; self.token_count = 0; Ok(()) }
    fn reject_invalid_encode_from_encode_validity_decision(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.err = ERROR_INVALID_ARGUMENT; self.token_count = 0; Ok(()) }
    fn reject_invalid_encode_from_encode_vocab_sync_decision(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.err = ERROR_INVALID_ARGUMENT; self.token_count = 0; Ok(()) }
    fn run_encode(&mut self, event: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.encode(event); Ok(()) }
    fn run_merge(&mut self, event: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.merge(event); Ok(()) }
    fn run_prepare(&mut self, event: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.prepare(event); Ok(()) }
    fn set_emit_result_empty(&mut self, _: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.token_count = 0; self.err = ERROR_OK; Ok(()) }
    fn symbols_absent(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(!self.symbols.iter().any(|symbol| symbol.alive)) }
    fn symbols_present(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.symbols.iter().any(|symbol| symbol.alive)) }
    fn sync_tables(&mut self, event: &EventEncodeRuntime<'_>) -> Result<(), ()> { self.tables_ready = event.vocab.tokens.len() <= MAX_VOCAB_ENTRIES && event.vocab.tokens.len() == event.vocab.scores.len(); self.vocab_key = event.vocab as *const _ as usize; self.err = if self.tables_ready { ERROR_OK } else { ERROR_MODEL_INVALID }; Ok(()) }
    fn table_sync_backend_error(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_BACKEND) }
    fn table_sync_invalid_argument_error(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_INVALID_ARGUMENT) }
    fn table_sync_model_invalid_error(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_MODEL_INVALID) }
    fn table_sync_ok(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.err == ERROR_OK) }
    fn table_sync_unclassified_error_code(&self, _: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(is_other(self.err)) }
    fn tables_missing(&self, event: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(!self.tables_ready || self.vocab_key != event.vocab as *const _ as usize) }
    fn tables_ready(&self, event: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.tables_ready && self.vocab_key == event.vocab as *const _ as usize) }
    fn text_empty(&self, event: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(event.text.is_empty()) }
    fn text_non_empty(&self, event: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(!event.text.is_empty()) }
    fn valid_encode(&self, event: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(event.token_capacity != 0 && event.token_capacity <= MAX_ENCODE_TOKENS) }
    fn vocab_changed(&self, event: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(self.vocab_key != event.vocab as *const _ as usize) }
    fn vocab_unchanged(&self, event: &EventEncodeRuntime<'_>) -> Result<bool, ()> { Ok(!self.vocab_changed(event)?) }
}

/// Synchronous bounded actor around the generated SPM machine.
pub struct TextEncodersSpm { machine: TextEncodersSpmStateMachine<TextEncodersSpmContext> }
impl Default for TextEncodersSpm { fn default() -> Self { Self::new() } }
impl TextEncodersSpm {
    #[must_use]
    pub fn new() -> Self { Self { machine: TextEncodersSpmStateMachine::new(TextEncodersSpmContext::default()) } }
    pub fn process<'event>(&mut self, mut request: EncodeRequest<'event>) -> bool { let accepted = { let event = EventEncodeRuntime::new(&request); self.machine.process_event(TextEncodersSpmEvents::EventEncodeRuntime(event)).is_ok() }; let (err, token_count) = { let ctx = self.machine.context(); let err = if accepted { ctx.err } else if ctx.err == ERROR_OK { ERROR_INVALID_ARGUMENT } else { ctx.err }; (err, ctx.token_count) }; self.machine.context().append_result(&mut request); if let Some(out) = request.error_out.as_deref_mut() { *out = err; } if err == ERROR_OK { if let Some(callback) = request.on_done { callback(EventsEncodingDone { request: None, token_count }); } } else if let Some(callback) = request.on_error { callback(EventsEncodingError { request: None, err }); } err == ERROR_OK }
    pub fn encode<'event>(&mut self, request: EncodeRequest<'event>) -> bool { self.process(request) }
...
mod tests {
    ...
    fn run(vocab: &SpmVocabulary<'_>, text: &str, output: &mut [i32]) -> (bool, i32, i32) {
        let mut count = -1;
        let mut error = -1;
        let request = EncodeRequest { vocab, text, token_ids: output, token_count_out: Some(&mut count), error_out: Some(&mut error), on_done: None, on_error: None };
