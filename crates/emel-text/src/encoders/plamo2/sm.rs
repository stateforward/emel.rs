...
/// Maximum vocabulary entries accepted by the maintained model loader.
...
impl<'a> RuntimeEncodeRuntime<'a> {
    ...
    pub fn new(request: EncodeRequest<'a>, context: &'a RefCell<EncodeContext>) -> Self {
        Self {
            ...
            context,
            data_len: Cell::new(0),
            emit_result_error: Cell::new(EncodeError::None),
            emit_result_token_count: Cell::new(0),
        }
    }
}
...
pub struct TextEncodersPlamo2Context {
    ...
    /// Identity of the vocabulary used to build the current tables.
    ///
    /// The actor is synchronous, so the request vocabulary is available on
    /// every action. Keep only its identity here; retaining the borrowed
    /// trait object would incorrectly extend a dispatch-local borrow.
    vocab_key: usize,
...
impl fmt::Debug for TextEncodersPlamo2Context {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
impl Default for TextEncodersPlamo2Context {
    fn default() -> Self {
        Self {
            tables_ready: false,
            vocab_key: 0,
...
sml! {
    TextEncodersPlamo2<'a> {
        ...
        "errored"_s <= "table_sync_result_decision"_s + completion<RuntimeEncodeRuntime>(RuntimeEncodeRuntime<'a>) [table_sync_model_invalid_error] / ensure_last_error_from_table_sync_result_decision,
...
impl TextEncodersPlamo2StateMachineContext for TextEncodersPlamo2Context {
    ...
    fn begin_encode_sync_vocab(&mut self, event: &RuntimeEncodeRuntime<'_>) -> Result<(), ()> {
        ...
        self.vocab_key = vocab_key(event.request.vocab);
    ...
    fn decode_result_backend_error(&self, event: &RuntimeEncodeRuntime<'_>) -> Result<bool, ()> {
        Ok(event.context.borrow().error == EncodeError::Backend)
    }
    fn decode_result_empty_ok(&self, event: &RuntimeEncodeRuntime<'_>) -> Result<bool, ()> {
        Ok(event.context.borrow().error == EncodeError::None && event.data_len.get() == 0)
    }
    ...
    fn decode_result_model_invalid_error(
        &self,
        event: &RuntimeEncodeRuntime<'_>,
    ) -> Result<bool, ()> {
        Ok(event.context.borrow().error == EncodeError::ModelInvalid)
    }
    fn decode_result_non_empty_ok(&self, event: &RuntimeEncodeRuntime<'_>) -> Result<bool, ()> {
        Ok(event.context.borrow().error == EncodeError::None && event.data_len.get() > 0)
    }
    fn decode_result_unclassified_error_code(
        ...
        Ok(event.context.borrow().error != EncodeError::None
            && !matches!(
                ...
                EncodeError::InvalidArgument | EncodeError::Backend | EncodeError::ModelInvalid
            ))
    }
    fn emit_result_failed(&self, event: &RuntimeEncodeRuntime<'_>) -> Result<bool, ()> {
        Ok(event.emit_result_error.get() != EncodeError::None)
    }
    fn emit_result_ok(&self, event: &RuntimeEncodeRuntime<'_>) -> Result<bool, ()> {
        Ok(event.emit_result_error.get() == EncodeError::None)
    }
    fn emit_tokens(&mut self, event: &RuntimeEncodeRuntime<'_>) -> Result<(), ()> {
        ...
        while pos < event.data_len.get() {
            ...
            if path.token_len == 0 || pos.saturating_add(path.token_len) > event.data_len.get() {
                error = EncodeError::Backend;
                break;
            }
            if path.token_id >= 0 {
                if count == capacity {
                    error = EncodeError::InvalidArgument;
                    break;
                }
        ...
        event
            .emit_result_token_count
            .set(if error == EncodeError::None { count } else { 0 });
    ...
    fn ensure_last_error_from_backtrace_result_decision(
        &mut self,
        event: &RuntimeEncodeRuntime<'_>,
    ) -> Result<(), ()> {
        ensure_last_error(event)
    ...
    fn invalid_encode(&self, event: &RuntimeEncodeRuntime<'_>) -> Result<bool, ()> {
        Ok(event.request.token_ids.borrow().is_empty()
            || event.request.vocab.token_count() > MAX_VOCAB_ENTRIES)
    }
    ...
    fn prepare_dp(&mut self, event: &RuntimeEncodeRuntime<'_>) -> Result<(), ()> {
        ...
        for index in 0..=len {
            self.scores[index] = BIG;
            self.paths[index] = PathEntry::default();
        }
        self.scores[len] = 0;
        Ok(())
    }
    ...
    fn run_dp(&mut self, event: &RuntimeEncodeRuntime<'_>) -> Result<(), ()> {
        let len = event.data_len.get();
        let vocab = event.request.vocab;
        const BIG: i64 = i64::MAX / 4;
        for pos in (0..len).rev() {
            let mut best_score = BIG;
            let mut best_path = PathEntry::default();
            for id in 0..vocab.token_count() {
                let Some(entry) = vocab.token(id) else {
                    continue;
                };
                if entry.kind == 6 || entry.text.is_empty() || !entry.score.is_finite() {
                    continue;
                }
                let mut end = pos;
                let mut matched = true;
                for cp in entry.text.chars() {
                    if end >= len || self.codepoints[end] != cp as u32 {
                        matched = false;
                        break;
                    }
                    end += 1;
                }
                if !matched || end == pos || self.scores[end] >= BIG {
                    continue;
                }
                let scaled = (f64::from(entry.score) * 10_000.0).round();
                let scaled = if scaled < f64::from(i32::MIN) {
                    i64::from(i32::MIN)
                ...
                let score = self.scores[end].saturating_sub(scaled);
                if score < best_score {
                    best_score = score;
                    best_path = PathEntry {
                        token_len: end - pos,
                        token_id: match i32::try_from(id) {
                            Ok(id) => id,
                            Err(_) => continue,
                        },
                    };
            ...
            if let Some(ch) = char::from_u32(self.codepoints[pos]) {
                let mut bytes = [0u8; 4];
                let width = ch.encode_utf8(&mut bytes).len();
                let fallback_ok = bytes[..width]
                    .iter()
                    .all(|byte| find_byte_token(vocab, *byte) > 0);
                let end = pos + 1;
                if fallback_ok && self.scores[end] < BIG {
                    let score = self.scores[end].saturating_add(10_000_000);
                    if score < best_score {
                        best_score = score;
                        best_path = PathEntry {
                            token_len: 1,
            ...
            if best_path.token_len == 0 {
                event.context.borrow_mut().error = EncodeError::Backend;
                continue;
            }
            self.scores[pos] = best_score;
            self.paths[pos] = best_path;
        }
        Ok(())
    }
    fn sync_tables(&mut self, event: &RuntimeEncodeRuntime<'_>) -> Result<(), ()> {
        self.vocab_key = vocab_key(event.request.vocab);
        let vocab = event.request.vocab;
        // The C++ model owns a contiguous entry array. A borrowed Rust view
        // can still advertise a count while failing an individual lookup;
        // classify that malformed model before declaring byte tables usable.
        for index in 0..vocab.token_count() {
            if vocab.token(index).is_none() {
                self.tables_ready = false;
                event.context.borrow_mut().error = EncodeError::ModelInvalid;
                return Ok(());
            }
        }
        let mut byte_tokens_complete = vocab.token_count() > 0;
        for byte in 0..=u8::MAX {
            byte_tokens_complete &= find_byte_token(vocab, byte) > 0;
        }
        self.tables_ready = byte_tokens_complete;
        event.context.borrow_mut().error = if self.tables_ready {
            EncodeError::None
        } else {
    ...
    fn tables_missing(&self, _event: &RuntimeEncodeRuntime<'_>) -> Result<bool, ()> {
        Ok(!self.tables_ready)
    }
    fn tables_ready(&self, event: &RuntimeEncodeRuntime<'_>) -> Result<bool, ()> {
        Ok(self.tables_ready && self.vocab_key == vocab_key(event.request.vocab))
    }
    ...
    fn vocab_changed(&self, event: &RuntimeEncodeRuntime<'_>) -> Result<bool, ()> {
        Ok(self.vocab_key != vocab_key(event.request.vocab))
    }
    fn vocab_unchanged(&self, event: &RuntimeEncodeRuntime<'_>) -> Result<bool, ()> {
        Ok(!self.vocab_changed(event)?)
    }
    fn backtrace_ok(&self, event: &RuntimeEncodeRuntime<'_>) -> Result<bool, ()> {
        Ok(event.context.borrow().error == EncodeError::None
            && (event.data_len.get() == 0 || self.paths[0].token_len > 0))
    }
    fn backtrace_failed(&self, event: &RuntimeEncodeRuntime<'_>) -> Result<bool, ()> {
        Ok(!self.backtrace_ok(event)?)
    }
    fn validate_backtrace(&mut self, event: &RuntimeEncodeRuntime<'_>) -> Result<(), ()> {
        if event.data_len.get() > 0 && self.paths[0].token_len == 0 {
            event.context.borrow_mut().error = EncodeError::Backend;
        }
        Ok(())
    }
    fn mark_backtrace_failed(&mut self, event: &RuntimeEncodeRuntime<'_>) -> Result<(), ()> {
        event.context.borrow_mut().error = EncodeError::Backend;
        Ok(())
    }
}
...
fn ensure_last_error(event: &RuntimeEncodeRuntime<'_>) -> Result<(), ()> {
    let mut context = event.context.borrow_mut();
    if context.error == EncodeError::None {
...
fn find_byte_token(vocab: &dyn VocabView, byte: u8) -> i32 {
    ...
    for index in 0..vocab.token_count() {
        let Some(entry) = vocab.token(index) else {
            continue;
        };
        ...
        if entry.kind == 6
            ...
            && bytes[5] == b'>'
            && bytes[3].to_ascii_uppercase() == digits[(byte >> 4) as usize]
            && bytes[4].to_ascii_uppercase() == digits[(byte & 0x0f) as usize]
        {
...
fn vocab_key(vocab: &dyn VocabView) -> usize {
    vocab as *const dyn VocabView as *const () as usize
}
...
pub struct TextEncodersPlamo2Actor<'a> {
    machine: TextEncodersPlamo2StateMachine<TextEncodersPlamo2Context>,
    _actor_lifetime: core::marker::PhantomData<&'a ()>,
}
...
impl<'a> TextEncodersPlamo2Actor<'a> {
    ...
    pub fn new() -> Self {
        Self {
            machine: TextEncodersPlamo2StateMachine::new(TextEncodersPlamo2Context::default()),
            _actor_lifetime: core::marker::PhantomData,
        }
    }
    pub fn process_event<'event>(&mut self, event: RuntimeEncodeRuntime<'event>) -> bool {
        let context = event.context;
        let error_callback = event.request.dispatch_error;
        let accepted = self
            .machine
            .process_event(TextEncodersPlamo2Events::RuntimeEncodeRuntime(event))
            .is_ok();
        let mut result = *context.borrow();
        if !accepted && result.error == EncodeError::None {
            context.borrow_mut().error = EncodeError::InvalidArgument;
            result.error = EncodeError::InvalidArgument;
        }
        if self.machine.is(&TextEncodersPlamo2States::Unexpected)
            || self.machine.context().unexpected
        {
            context.borrow_mut().error = EncodeError::InvalidArgument;
            result.error = EncodeError::InvalidArgument;
        }
    ...
    /// Returns the last request error from the caller-owned result context.
    #[must_use]
    pub fn last_error(&self, context: &RefCell<EncodeContext>) -> EncodeError {
        context.borrow().error
    }
    /// Dispatches an explicit sequencing violation and records invalid input.
    pub fn process_unexpected(&mut self) -> bool {
        self.machine.set_state(TextEncodersPlamo2States::Unexpected);
        self.machine.context_mut().unexpected = true;
        false
    }
    /// Returns generated state inspection data.
    #[must_use]
    pub fn state(&self) -> &TextEncodersPlamo2States {
        self.machine.state()
    }
    /// Reports whether the actor is in `state`.
    #[must_use]
    pub fn is(&self, state: &TextEncodersPlamo2States) -> bool {
        self.machine.is(state)
    }
    /// Returns generated context inspection data.
    #[must_use]
    pub fn context(&self) -> &TextEncodersPlamo2Context {
