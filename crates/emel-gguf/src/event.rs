...
/// Read a string into caller-owned fixed storage without allocating.
#[derive(Debug)]
pub struct ReadStringInto<'a, const N: usize> {
    pub(crate) key: &'a [u8],
    pub(crate) destination: &'a mut [u8; N],
    pub(crate) length: usize,
    pub(crate) result: Result<Option<usize>, QueryError>,
}

impl<'a, const N: usize> ReadStringInto<'a, N> {
    /// Creates a fixed-capacity string query over caller-owned storage.
    #[must_use]
    pub const fn new(key: &'a [u8], destination: &'a mut [u8; N]) -> Self {
        Self {
            key,
            destination,
            length: 0,
...
impl<const N: usize> sealed::Sealed for ReadStringInto<'_, N> {}
impl<const N: usize> Event for ReadStringInto<'_, N> {
    type Output = Result<Option<usize>, QueryError>;
    fn dispatch(mut self, actor: &mut Loader) -> Self::Output {
