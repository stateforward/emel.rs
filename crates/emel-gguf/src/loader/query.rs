...
use crate::event::{
    ...
    ReadStringArrayMetrics, ReadStringInto, ReadUnsigned, ReadUnsignedArrayElement,
    ReadUnsignedArrayMetrics, Storage, StringArrayMetrics, UnsignedArrayMetrics, VisitBoolArray,
    VisitF32Array, VisitStringArray, VisitUnsignedArray, WithByteArray, WithString,
...
impl<const N: usize> Operation for ReadStringInto<'_, N> {
    fn key(&self) -> &[u8] {
        self.key
    }
    fn decision(&self, entry: Entry<'_>) -> Decision {
    ...
    fn apply_string(&mut self, value: &[u8]) {
        self.destination[..value.len()].copy_from_slice(value);
        self.length = value.len();
        self.result = Ok(Some(self.length));
    }
}
...
mod tests {
    ...
    #[test]
    fn fixed_string_query_classifies_capacity_before_copy() {
        let storage = Storage {
            source: std::sync::Arc::from([]),
            kv_arena: vec![b'k', 3, b'a', b'b', b'c'],
            kv_entries: vec![super::super::KvEntry {
                key_offset: 0,
                key_length: 1,
                value_offset: 1,
        ...
        let mut event = ReadStringInto::new(b"k", &mut destination);
        process(true, Some(&storage), 1, &mut event);
        assert_eq!(event.result, Err(QueryError::Range));
