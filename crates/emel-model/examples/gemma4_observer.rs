...
use emel_model::gemma4::{Gemma4, Parameters, load_hparams};
use emel_model::generation::QuantizedStageFamily;
use emel_model::generation::quantized_path::Resolver;
use emel_model::generation::{AttentionQkNormRoute, tensor_type_name};
use emel_tensor as _;
use emel_token as _;
use sml as _;

const SOURCE_COMMIT: &str = "843a117386ef17dc5a50549bbfc821074c2141d6";
const DETAIL_HEADER_BLOB: &str = "UNAVAILABLE";
const DETAIL_IMPLEMENTATION_BLOB: &str = "UNAVAILABLE";
...
fn main() {
    ...
    match args.as_slice() {
        ...
        _ => {
            eprintln!("usage: gemma4_observer --fixture GEMMA4|--benchmark ITER RUNS WARMUP");
            std::process::exit(2);
        }
    }
}
...
fn build(catalog: Catalog, model: ModelIdentity, parameters: Parameters) -> Gemma4 {
    let mut actor = Gemma4::new(
        catalog,
        Resolver::new(),
        emel_model::gemma4::event::Storage::with_block_capacity(
...
fn print_contract(actor: &mut Gemma4, label: &str) {
    ...
    println!(
        "gemma4_case={label} block_count={} tensor_count={} workspace={} prefill={} decode={} uses_attention={} qk_norm_headwise_rms={} key_length={} value_length={} rope_dim={}",
        ...
        u8::from(block.layer().qk_norm_route() == AttentionQkNormRoute::HeadwiseRms),
...
fn fixture(path: &str) {
    ...
    let _parameters = load_hparams(&mut loader).expect("Gemma4 hparams");
    let storage = CatalogStorage::from_gguf(&mut loader, parsed).expect("catalog storage");
    let (catalog, model) = seal(storage);
    let mut actor = build(catalog, model, parameters(1));
