...
/// Loads the ten required metadata keys consumed by pinned Qwen3 `load_hparams`.
///
/// The source-fixed output width, tied-embedding policy, `RoPE` dimension, and
/// pair-layout facts are derived only after every required key is decoded.
///
/// # Errors
///
/// Returns a typed metadata-access error for missing, malformed, wrong-kind, or
/// out-of-range required metadata.
