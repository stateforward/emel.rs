...
#[test]
fn out_of_order_lifecycle_requests_recover_after_typed_child_errors() {
    let (mut actor, model) = actor();
    assert!(format!("{actor:?}").starts_with("Llama { .. }"));
    actor
        .process_event(ContractBegin::new(b"llama", model, parameters()))
        .unwrap();
    actor.process_event(BlockBuild::new(0)).unwrap();
    ...
    for family in QuantizedStageFamily::ALL {
        assert!(matches!(
            actor.process_event(StageAudit::new(family)),
            Err(Error::InvalidRequest | Error::ModelInvalid)
        ));
    }
    assert!(matches!(
    ...
    assert!(matches!(
        actor.process_event(PlanBuild),
        Err(Error::InvalidRequest | Error::ModelInvalid)
    ));
    assert!(matches!(
        actor.process_event(ContractVisit),
        Err(Error::InvalidRequest | Error::ModelInvalid)
    ));
    assert!(matches!(
    ...
    for index in [-1_i32, 0, 1] {
        assert!(matches!(
            actor.process_event(BlockVisit::new(index)),
            Err(Error::InvalidRequest)
        ));
    }

    actor.process_event(ContractReset).unwrap();
}
    ...
    for key in HPARAM_KEYS {
        let mut loader = crate::loader::test_gguf::load(
            crate::loader::test_gguf::Fixture::new()
                .string(key.as_bytes(), b"not-a-number")
                .build(),
        );
        assert!(
            super::load_hparams(&mut loader).is_err(),
            "{key} must reject the wrong wire kind",
        );
    }
}
