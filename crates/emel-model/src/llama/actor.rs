//! Public Llama actor and private same-RTC dispatch runtimes.
...
pub struct Llama {
    machine: LlamaMachineStateMachine<Context>,
}
...
impl Llama {
    ...
    pub fn new(
        ...
        Ok(Self {
            machine: LlamaMachineStateMachine::new(Context {
                builder,
...
impl Context {
    ...
    fn topology_values(&self) -> Option<(u32, u64)> {
        ...
        let tensors = blocks
            .checked_mul(BLOCK_TENSOR_COUNT)?
...
impl LlamaMachineStateMachineContext for Context {
    ...
    fn effect_global_child(&mut self, event: BeginRuntime<'_>) -> Result<(), ()> {
        event.global_result.set(self.builder.process_event(
            generation::event::GlobalBindings::new(TOKEN_EMBEDDING_NAME, OUTPUT_NORM_NAME, false),
        ));
        Ok(())
    }
