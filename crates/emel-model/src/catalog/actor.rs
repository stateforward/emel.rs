...
use super::event::{
    self, BindStorageError, Error, ModelDescriptor, ModelIdentity, PrefixScanSummary, Storage,
    TensorBindingStatus, TensorDescriptor, TensorId,
};
use super::sm::{
...
#[derive(Clone, Copy)]
pub(super) struct ScanPrefixRuntime<'a> {
    identity: ModelIdentity,
    prefix: &'a [u8],
    result: &'a Cell<Result<PrefixScanSummary, Error>>,
}

#[derive(Clone, Copy)]
pub(super) struct ValidateTensorShapeRuntime<'a> {
    identity: ModelIdentity,
    name: &'a [u8],
    dimensions: &'a [i64],
    result: &'a Cell<Result<bool, Error>>,
}
...
impl Catalog {
    ...
    pub(crate) fn scan_prefix(
        &mut self,
        event: event::ScanPrefix<'_>,
    ) -> Result<PrefixScanSummary, Error> {
        let result = Cell::new(Err(Error::Internal));
        let runtime = ScanPrefixRuntime {
            identity: event.identity,
            prefix: event.prefix,
            result: &result,
        };
        self.machine
            .process_event(CatalogMachineEvents::ScanPrefix(runtime))
            .map_err(|_| Error::Internal)?;
        result.get()
    }

    pub(crate) fn validate_tensor_shape(
        &mut self,
        event: event::ValidateTensorShape<'_>,
    ) -> Result<bool, Error> {
        let result = Cell::new(Err(Error::Internal));
        let runtime = ValidateTensorShapeRuntime {
            identity: event.identity,
            name: event.name,
        ...
        self.machine
            .process_event(CatalogMachineEvents::ValidateTensorShape(runtime))
            .map_err(|_| Error::Internal)?;
        result.get()
    }
...
impl CatalogMachineStateMachineContext for Context {
    ...
    fn guard_scan_wrong(&self, event: &ScanPrefixRuntime<'_>) -> Result<bool, ()> {
        Ok(event.identity.owner != self.owner)
    }

    fn guard_scan_stale(&self, event: &ScanPrefixRuntime<'_>) -> Result<bool, ()> {
        Ok(event.identity.owner == self.owner && event.identity.generation != self.generation)
    }

    fn guard_scan_valid(&self, event: &ScanPrefixRuntime<'_>) -> Result<bool, ()> {
        Ok(event.identity.owner == self.owner && event.identity.generation == self.generation)
    }

    fn guard_shape_wrong(&self, event: &ValidateTensorShapeRuntime<'_>) -> Result<bool, ()> {
        Ok(event.identity.owner != self.owner)
    }

    fn guard_shape_stale(&self, event: &ValidateTensorShapeRuntime<'_>) -> Result<bool, ()> {
        Ok(event.identity.owner == self.owner && event.identity.generation != self.generation)
    }

    fn guard_shape_valid(&self, event: &ValidateTensorShapeRuntime<'_>) -> Result<bool, ()> {
        Ok(event.identity.owner == self.owner && event.identity.generation == self.generation)
    }
    ...
    fn effect_scan_wrong(&mut self, event: ScanPrefixRuntime<'_>) -> Result<(), ()> {
        event.result.set(Err(Error::WrongModelIdentity));
        Ok(())
    }

    fn effect_scan_stale(&mut self, event: ScanPrefixRuntime<'_>) -> Result<(), ()> {
        event.result.set(Err(Error::StaleModelIdentity));
        Ok(())
    }

    fn effect_scan_storage_unavailable(&mut self, event: ScanPrefixRuntime<'_>) -> Result<(), ()> {
        event.result.set(Err(Error::StorageUnavailable));
        Ok(())
    }

    fn effect_scan_prefix(&mut self, event: ScanPrefixRuntime<'_>) -> Result<(), ()> {
        let storage = self.storage.as_ref().expect("sealed storage");
        let mut count = 0_u32;
        let mut all_bound = true;
        let mut all_geometry_valid = true;
        for ordinal in 0..storage.tensor_count {
            let record = storage.records[ordinal];
            let name = storage::range(&storage.names, record.name_offset, record.name_length)
                .expect("seal guard validates name range");
            if name.starts_with(event.prefix) {
                count = count.saturating_add(1);
                let ordinal = u32::try_from(ordinal).expect("catalog fits u32");
                all_bound &= self.ordinal_bound(ordinal);
                all_geometry_valid &= self.ordinal_geometry_valid(ordinal);
            }
        }
        event.result.set(Ok(PrefixScanSummary::new(
            count,
            all_bound,
            all_geometry_valid,
        )));
    ...
    fn effect_shape_wrong(&mut self, event: ValidateTensorShapeRuntime<'_>) -> Result<(), ()> {
        event.result.set(Err(Error::WrongModelIdentity));
        Ok(())
    }

    fn effect_shape_stale(&mut self, event: ValidateTensorShapeRuntime<'_>) -> Result<(), ()> {
        event.result.set(Err(Error::StaleModelIdentity));
        Ok(())
    }

    fn effect_shape_storage_unavailable(
        &mut self,
        event: ValidateTensorShapeRuntime<'_>,
    ) -> Result<(), ()> {
        event.result.set(Err(Error::StorageUnavailable));
        Ok(())
    }

    fn effect_validate_tensor_shape(
        &mut self,
        event: ValidateTensorShapeRuntime<'_>,
    ) -> Result<(), ()> {
        let valid = self.find_ordinal(event.name).is_some_and(|ordinal| {
            let Some(storage) = self.storage.as_ref() else {
                return false;
            };
            let Some(record) = usize::try_from(ordinal)
                .ok()
                .and_then(|index| storage.records.get(index))
            else {
                return false;
            };
            let Ok(rank) = usize::try_from(record.dimension_count) else {
                return false;
            };
            rank == event.dimensions.len()
                && rank > 0
                && rank <= record.dimensions.len()
                && record.binding_present
                && record.data_size > 0
                && record.dimensions[..rank] == *event.dimensions
        });
        event.result.set(Ok(valid));
        Ok(())
    }
...
impl Context {
    ...
    fn ordinal_geometry_valid(&self, ordinal: u32) -> bool {
        let Some(record) = usize::try_from(ordinal).ok().and_then(|ordinal| {
            self.storage
                .as_ref()
                .and_then(|storage| storage.records.get(ordinal))
