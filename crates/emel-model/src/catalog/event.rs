...
/// Summary of tensors whose canonical names begin with a supplied prefix.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct PrefixScanSummary {
    count: u32,
    all_bound: bool,
    all_geometry_valid: bool,
}

impl PrefixScanSummary {
    #[must_use]
    ...
    #[must_use]
    pub const fn all_geometry_valid(self) -> bool {
        self.all_geometry_valid
    }

    pub(super) const fn new(count: u32, all_bound: bool, all_geometry_valid: bool) -> Self {
        Self {
            count,
            all_bound,
            all_geometry_valid,
...
/// Scans canonical tensor names without exposing catalog storage or identities.
#[derive(Clone, Copy, Debug)]
...
/// Validates one tensor's exact name, rank, and dimensions through the catalog actor.
#[derive(Clone, Copy, Debug)]
pub struct ValidateTensorShape<'a> {
    pub(super) identity: ModelIdentity,
    pub(super) name: &'a [u8],
    pub(super) dimensions: &'a [i64],
}

impl<'a> ValidateTensorShape<'a> {
    #[must_use]
    pub const fn new(identity: ModelIdentity, name: &'a [u8], dimensions: &'a [i64]) -> Self {
        Self {
            identity,
            name,
            dimensions,
...
impl<'a> ScanPrefix<'a> {
    #[must_use]
    pub const fn new(identity: ModelIdentity, prefix: &'a [u8]) -> Self {
...
simple_event!(ValidateTensorShape<'_>, Result<bool, Error>, validate_tensor_shape);
