...
fn generated_machine_storage_regions_and_state_are_inaccessible() {
    for (case, source) in [
        ...
        (
            "state",
            "fn main() { let catalog = emel_model::catalog::Catalog::try_new().unwrap(); let actor = emel_model::generation::Builder::new(catalog, emel_model::generation::quantized_path::Resolver::new()); let _ = actor.state(); }",
        ),
    ] {
