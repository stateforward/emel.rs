//! External Lfm2 actor composition and generated-state privacy checks.
...
fn check_project(case: &str, source: &str) -> Output {
    ...
    let output = Command::new(env!("CARGO"))
        ...
        .env(
            "CARGO_TARGET_DIR",
            root.join("../../target/model-lfm2-privacy"),
        )
        .current_dir(&project)
        .output()
        .unwrap();
...
fn public_parameters_and_events_compile() {
    let output = check_project(
        "public",
        "use emel_model::lfm2::{Parameters, event::{ContractBegin, HPARAM_KEYS}};\n\
...
fn generated_machine_context_and_state_are_inaccessible() {
    for (case, source) in [
        (
            "machine",
            "use emel_model::lfm2::sm::Lfm2MachineStateMachine; fn main() {}",
        ),
        (
            "context",
            "use emel_model::lfm2::actor::Context; fn main() {}",
        ),
        (
            "state",
            "fn inspect(value: emel_model::lfm2::Lfm2) { let _ = value.state(); } fn main() {}",
        ),
    ] {
