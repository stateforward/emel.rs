//! External Gemma4 actor composition and generated-state privacy checks.
...
fn check_project(case: &str, source: &str) -> Output {
    ...
    let project = std::env::temp_dir().join(format!(
        "emel-model-gemma4-privacy-{case}-{}",
        std::process::id()
    ));
    ...
    let output = Command::new(env!("CARGO"))
        ...
        .env(
            "CARGO_TARGET_DIR",
            root.join("../../target/model-gemma4-privacy"),
        )
        .current_dir(&project)
        .output()
        .unwrap();
...
fn public_parameters_and_events_compile() {
    let output = check_project(
        "public",
        "use emel_model::gemma4::{Parameters, event::{ContractBegin, HPARAM_KEYS}};\n\
...
fn generated_machine_context_and_state_are_inaccessible() {
    for (case, source) in [
        (
            "machine",
            "use emel_model::gemma4::sm::Gemma4MachineStateMachine; fn main() {}",
        ),
        (
            "context",
            "use emel_model::gemma4::actor::Context; fn main() {}",
        ),
        (
            "state",
            "fn inspect(value: emel_model::gemma4::Gemma4) { let _ = value.state(); } fn main() {}",
        ),
    ] {
